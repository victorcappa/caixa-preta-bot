...
const GUESS_WHO_FALLBACK_QUESTIONS = [
  "Essa pessoa era conhecida principalmente por politica?",
  "Essa pessoa era brasileira?",
  "Essa pessoa ocupou cargo publico?",
  "Essa pessoa foi presidente?",
  "Essa pessoa era homem?",
  "Essa pessoa morreu depois de 2000?",
  "Essa pessoa esteve envolvida em escandalo publico?",
  "Essa pessoa era conhecida nacionalmente?",
  "Essa pessoa viveu no seculo XX?",
  "Essa pessoa morreu de forma violenta ou inesperada?",
  "Essa pessoa era ligada a dinheiro publico?",
  "Essa pessoa ficou famosa antes da internet?"
];
...
function nextGuessWhoFallbackQuestion(guessWho = {}) {
  const asked = new Set((guessWho.knownFacts || [])
    .map((fact) => normalizeQuestionKey(fact.question || "")));
  const pending = normalizeQuestionKey(guessWho.pendingQuestion || "");

  return GUESS_WHO_FALLBACK_QUESTIONS.find((question) => {
    const key = normalizeQuestionKey(question);
    return key && key !== pending && !asked.has(key);
  }) || "Essa pessoa era conhecida publicamente?";
}
...
export function enforceGuessWhoTurn(turn = {}, suitcaseState = createInitialSuitcaseState()) {
  ...
  if (validQuestion && isGuessWhoClosedQuestion(turn.text || validQuestion)) {
    return {
      ...turn,
      suitcase: {
        ...(move || {}),
...
export function suitcaseContextBlock(state = {}) {
  ...
  return [
    ...
    "Mala 1 / guess_who: faca EXCLUSIVAMENTE perguntas respondiveis por sim, nao ou nao sei. Nunca peca categoria, uma palavra, local/nacional, escolha entre opcoes ou permissao para palpite. A Caixa decide a proxima pergunta e decide quando palpitar. Comentarios nao contam, mas nao podem substituir a pergunta objetiva.",
