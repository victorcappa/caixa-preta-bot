...
export function isGuessWhoClosedQuestion(text = "") {
  ...
  if (/\b(escolha|escolher|mala|esquerda|centro|direita|continuar|qual|quais|categoria|uma palavra|quer|posso|prefere|me diga|me diz|conte|conta|local ou nacional|financeir[ao]s?,?\s+morais|conflitos de interesse)\b/.test(normalized)) {
    return false;
  }
...
export function findInstagramParticipantByName(name = "") {
  const key = normalizeText(name).replace(/[^a-z0-9]+/g, " ").trim();
  if (!key) return null;
  const person = readInstagramParticipants().find((candidate) => (
    normalizeText(candidate?.name).replace(/[^a-z0-9]+/g, " ").trim() === key
  ));
  if (!person) return null;
  return {
    id: `${person.id || ""}`.slice(0, 80),
    name: `${person.name || ""}`.slice(0, 100),
    instagramHandle: `${person.instagramHandle || ""}`.slice(0, 40),
    instagramUrl: `${person.instagramUrl || ""}`.slice(0, 500)
  };
}
...
export function interpretSuitcaseContent(input = "") {
  ...
  const hasWrittenThing = /\b(algo escrito|coisa escrita|papel escrito|tem escrito|esta escrito|esta escrita)\b/.test(text);
  ...
  if (
    (hasPaper && hasName) ||
    /\b(?:tem|achei|encontrei|vi|apareceu)\s+(?:um\s+)?nome\b/.test(text) ||
    /\bnome escrito\b/.test(text) ||
    /\bpessoa escrita\b/.test(text) ||
    /\bpapel\s+(?:tem|com|traz)\s+(?:um\s+)?nome\b/.test(text)
  ) {
    return { type: SUITCASE_EXPERIENCES.guessWho, confidence: 0.92, reason: "paper_name" };
  }

  if (hasPaper || hasWrittenThing) {
    return { type: "needs_person_confirmation", confidence: 0.65, reason: "paper_needs_person_confirmation" };
  }
...
export function advanceSuitcases(state = createInitialSuitcaseState(), input = "") {
  ...
  if (base.phase === SUITCASE_PHASES.waitingContent) {
    const previousNeedsPersonConfirmation = base.lastInterpretation?.reason === "paper_needs_person_confirmation";
    if (previousNeedsPersonConfirmation && normalizeYesNo(input) === "sim") {
      return startGuessWho({
        ...base,
        lastInterpretation: {
          type: SUITCASE_EXPERIENCES.guessWho,
          confidence: 0.88,
          reason: "person_confirmation"
        }
      });
    }
...
function isGuessWhoStopRequest(input = "") {
  const text = normalizeText(input);
  return /\b(?:para|pare|parar|chega|encerra|encerrar|cancela|cancelar|muda de jogo|troca de jogo|outro jogo)\b/.test(text);
}

function extractGuessWhoReveal(input = "") {
  ...
  const explicitPatterns = [
    /\b(?:a\s+)?resposta\s+(?:e|eh|\u00e9|era)\s+(.+?)\s*[.!?]*$/iu,
    /\b(?:o\s+)?nome\s+(?:e|eh|\u00e9|era)\s+(.+?)\s*[.!?]*$/iu,
    /\b(?:no|do|o)\s+papel\b.*\b(?:e|eh|\u00e9|era|estava escrito)\s+(.+?)\s*[.!?]*$/iu,
    /\bestava escrito\s+(.+?)\s*[.!?]*$/iu,
    /\b(?:era|e|eh|\u00e9)\s+([A-Z\u00c0-\u00d6\u00d8-\u00de][\p{L}'-]+(?:\s+[A-Z\u00c0-\u00d6\u00d8-\u00de][\p{L}'-]+){0,3})\s*[.!?]*$/u
  ];

  for (const pattern of explicitPatterns) {
    const match = text.match(pattern);
    const candidate = match?.[1]?.trim().replace(/^a\s+/i, "").replace(/^o\s+/i, "");
    if (candidate && !/^(sim|nao|n\u00e3o|talvez|nao sei|n\u00e3o sei|isso|exato|correto)$/i.test(candidate)) {
...
function advanceGuessWho(state, input, stateBefore) {
  ...
  const revealedAnswer = extractGuessWhoReveal(input);

  if (!guessWho || guessWho.finished) {
    return { state, result: null, events: [] };
  }

  if (isGuessWhoStopRequest(input)) {
    return finishSuitcases({
      ...
      result: "stopped"
    }, "stopped", { source: "public" });
  }

  if (revealedAnswer && !(guessWho.pendingGuess && answer === "sim")) {
    return finishSuitcases({
      ...state,
      guessWho: {
        ...guessWho,
      ...
      result: "audience_win"
    }, "audience_win", { source: "public_reveal" });
  }
  ...
  if (guessWho.pendingGuess) {
    ...
    const guesses = [...guessWho.guesses, { guess: guessWho.pendingGuess, hit }].slice(-20);
    const result = hit ? "machine_win" : null;
    const nextGuessWho = {
      ...guessWho,
      guesses,
      pendingGuess: null,
      rejectedHypotheses: hit
        ...
        : [...guessWho.rejectedHypotheses, guessWho.pendingGuess].slice(-20),
      active: !result,
      finished: Boolean(result),
      winner: hit ? "machine" : null
    };
    const next = {
      ...state,
    ...
    return { state: logged, result: { type: "guess_answer", answer, hit, exhausted: false }, events: [] };
  }

  const questionCount = guessWho.questionCount + (guessWho.pendingQuestion ? 1 : 0);
  const nextGuessWho = {
    ...guessWho,
    questionCount,
    pendingQuestion: null,
...
export function suitcaseContextBlock(state = {}) {
  ...
  return [
    ...
    "Se phase = WAITING_FOR_SUITCASE_CONTENT, pergunte naturalmente o que tem dentro. Se lastInterpretation.reason = paper_needs_person_confirmation, faca no maximo uma pergunta: \"e o nome de uma pessoa?\". Se a resposta confirmar pessoa/nome, a mala acabou como assunto e Maria Antonieta comeca.",
    "Papel, bilhete, cartao, folha, algo escrito ou nome escrito dentro da mala sao gatilhos dramaticos; nao pergunte se o publico quer jogar, se quer que voce leia, se quer que voce tente, nem peca esquerda/centro/direita depois disso.",
    ...
    "Palpite errado e informacao, nao derrota: elimine o candidato, compare nomes parecidos, mude a pergunta se a arvore estiver ruim e continue. Nunca volte para malas, nunca pergunte se querem continuar e nunca faca metralhadora de nomes.",
