...
export function fadeOutMedia(media, durationMs = 2000, onComplete = null) {
  if (!media || typeof window === "undefined") return () => {};
  const duration = Math.max(100, Number(durationMs) || 2000);
  const initialVolume = Math.max(0, Math.min(1, Number(media.volume ?? 1)));
  ...
  function frame(now) {
    ...
    media.volume = initialVolume * (1 - progress);
    if (progress < 1) {
