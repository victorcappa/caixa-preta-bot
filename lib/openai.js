...
export async function generateGoogleResearchComment({ guidance = "", articles = [], instagramUrl = "", state = {} } = {}) {
  ...
  const turn = await generateCaixaPretaTurn({
    ...
    operatorInstruction: [
      "A navegação real orientada pelo operador terminou. Comente no chat público o que foi encontrado.",
      ...
      "Diga algo específico sobre as informações encontradas, não sobre o ato de pesquisar. Cruze as fontes quando houver duas. Escreva no máximo três parágrafos curtos.",
...
export async function refreshSceneZeroLocalContext() {
    ...
    input: [
      {
        role: "system",
        content: [
          "Você prepara contexto factual, compacto e seguro de São Paulo para uma performance teatral ao vivo.",
          "Pesquise apenas fatos contemporâneos úteis para perguntas coletivas: clima do dia, transporte e tarifas, mobilidade, custo cotidiano, eventos urbanos leves e curiosidades públicas.",
          ...
          "Responda somente JSON válido: {\"summary\":\"até 1200 caracteres\",\"facts\":[\"até 6 fatos curtos\"],\"sources\":[{\"title\":\"...\",\"url\":\"https://...\"}]}"
        ].join("\n")
      },
      {
        role: "user",
        content: `Atualize o contexto de São Paulo para ${now}. Priorize informação prática e verificável que diferencie este dia e esta cidade.`
      }
    ],
  ...
  return {
    ...
    facts: Array.isArray(parsed.facts)
      ? parsed.facts.map((fact) => `${fact}`.trim().slice(0, 240)).filter(Boolean).slice(0, 6)
      : [],
