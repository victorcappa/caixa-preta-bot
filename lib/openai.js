...
import { sceneZeroEmergenceCueForMessage } from "@/data/scene-zero-emergence";
...
function modelVisibleConversation(conversation = []) {
  return conversation.filter((message) => !sceneZeroEmergenceCueForMessage(message));
}

function conversationMessages(conversation) {
  return modelVisibleConversation(conversation).slice(-20).map((message) => ({
...
export async function generateCaixaPretaTurn({
  ...
  const modelConversation = modelVisibleConversation(state.conversation);
  ...
  const activeMemoryContext = [
    userMessage,
    operatorInstruction,
    ...modelConversation.slice(-6).map((message) => message.content)
  ].filter(Boolean).join("\n");
  const trainingContext = buildTrainingContextBlock({
    mode: state.mode,
    memories: state.memories,
    conversation: modelConversation.slice(-8),
    userMessage,
    operatorInstruction
  });
  ...
  const interactionHistoryContext = allowPerformance && state.mode === "host"
    ? buildInteractionHistoryBlock(modelConversation)
    : "";
  ...
  const input = [
    ...
    ...conversationMessages(modelConversation)
  ];
...
CONTEXTO RECENTE:
${modelVisibleConversation(conversation).map((message) => `${message.role === "assistant" ? "CAIXA" : "PUBLICO"}: ${message.content}`).join("\n")}
