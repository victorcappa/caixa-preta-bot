...
export async function generateAudienceWarmupSurprise({ state, surpriseCount = 1 } = {}) {
  ...
  const level = profile.intensity === "light"
    ? "quase normal e corporal, com ridículo leve"
    : profile.intensity === "medium"
      ? "social: começa a dividir a sala e fazer as pessoas se observarem"
      : {
        1: "claramente estranho, mas ainda lúdico",
        2: "mais íntimo e comprometedor, sem exigir revelação sensível",
        3: "provocativo: faz a sala escolher e julgar situações, não características protegidas",
        4: "inquietante, como se a máquina já conhecesse a rotina da sala",
        5: "muito estranho e levemente corrompido, confundindo cidade, notícia, memória e presença sem perder a ação executável"
      }[profile.surpriseLevel];
  ...
  const response = await getClient().responses.create({
    ...
    input: [{
      role: "system",
      content: [
        "Crie uma única instrução curta para aquecer a plateia de uma peça em São Paulo.",
        ...
        "A ação deve ser segura, coletiva e executável da cadeira.",
        "Responda somente JSON válido no formato: {\"text\":\"...\",\"action\":\"clap|hand|hands|feet|word|sound|silence|look|stand|sit|point|snap|lean|turn|eyes|laugh\",\"countdown\":false}"
      ].join("\n")
    }, {
      role: "user",
      content: [
