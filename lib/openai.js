...
export async function generateAudienceWarmupSurprise({ state, surpriseCount = 1 } = {}) {
  const level = surpriseCount <= 1
    ? "quase normal"
    : surpriseCount === 2
      ? "estranho, mas ainda plausível"
      : surpriseCount === 3
        ? "claramente estranho, conectando fatos que normalmente não seriam conectados"
        : surpriseCount === 4
          ? "inquietante, como se a máquina já conhecesse a rotina da sala"
          : "muito estranho e levemente corrompido, confundindo cidade, notícia, memória e presença sem perder a ação executável";
