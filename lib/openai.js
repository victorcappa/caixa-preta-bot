...
function parseTurnEnvelope(rawText = "") {
  ...
  try {
    ...
    const safeEvents = filterAgentPerformanceEvents(events, publicText);
  ...
  if (operatorInstruction) {
    ...
    input.push({
      ...
      content: allowPerformance
        ? `CONVERSA - PUBLICO:\n${userMessage}\n\nResponda somente no envelope JSON estruturado pedido. Se nao quiser evento, use "events": []. Se nao quiser iniciar jogo autonomo, use "game": null. O campo "text" deve reagir a fala recebida e, salvo encerramento real do operador, deixar continuidade concreta: nova acao, regra, alvo, escolha, pausa temporal ou consequencia. Nao termine como validacao administrativa seca.`
        : `CONVERSA - PUBLICO:\n${userMessage}`
    });
  }
