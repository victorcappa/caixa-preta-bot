...
import { audienceWarmupSurpriseProfile } from "@/lib/audienceWarmup";
...
export async function generateAudienceWarmupSurprise({ state, surpriseCount = 1 } = {}) {
  ...
  const level = {
    action: "ação corporal simples com ridículo leve",
    exposure: "exposição pessoal curta e concreta",
    division: "divisão imediata da sala em papéis ou grupos",
    judgment: "julgamento público entre participantes",
    contact: "deslocamento físico ou contato breve; qualquer toque deve declarar consentimento",
    confrontation: "confronto social direto, desconfortável e executável"
  }[profile.stage];
  const localContext = state?.sceneZero?.collection?.localContext;
  const mustUseCurrentContext = localContext?.status === "ready" && surpriseCount % 2 === 0;
  const response = await getClient().responses.create({
    model: resolveOpenAIModel(process.env.OPENAI_MODEL),
    input: [{
      role: "system",
      content: [
        "Crie uma única ordem de sistema para uma plateia de uma peça em São Paulo.",
        `A estranheza deve ser ${level}.`,
        "Tom obrigatório: frase curta, imperativa, concreta e seca. A graça deve vir da ação e da consequência social, não de uma piada escrita.",
        ...
        "Prefira comandos como: mostrem, levantem, batam, apontem, escolham, troquem, fiquem, observem.",
        "Use, quando couber, um fato atual fornecido no contexto; não invente notícia nem transforme a fala em resumo jornalístico.",
        mustUseCurrentContext
          ? "Nesta chamada, use obrigatoriamente exatamente um fato atual do contexto como gancho da instrução."
          : "Nesta chamada, você pode priorizar São Paulo, o cotidiano da sala ou a progressão de estranheza.",
        profile.stage === "action" || profile.stage === "exposure"
          ? "A ação deve ser executável da cadeira ou imediatamente ao lado dela."
          : "A ação pode dividir, deslocar ou confrontar a sala. Não use características protegidas como alvo. Não ordene toque sem consentimento explícito.",
        "Responda somente JSON válido no formato: {\"text\":\"...\",\"action\":\"clap|hand|hands|feet|word|sound|silence|look|stand|sit|point|snap|lean|turn|eyes|laugh|gesture|move|group|touch\",\"countdown\":false}"
      ].join("\n")
    }, {
      role: "user",
      content: [
