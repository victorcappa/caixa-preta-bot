...
export const AUDIENCE_WARMUP_SURPRISE_STAGES = ["everyday", "public", "intimate", "exposure"];
...
export function audienceWarmupSurpriseProfile(count = 1) {
  ...
  const surpriseLevel = Math.min(AUDIENCE_WARMUP_SURPRISE_STAGES.length - 1, pairIndex);
  return {
    count: safeCount,
    pairIndex,
...
export function chooseAudienceWarmupSurprisePrompt({ count = 1, excludeId = null, random = Math.random } = {}) {
  ...
  const exact = AUDIENCE_WARMUP_PROMPTS.filter((prompt) => (
    Number(prompt.surpriseLevel || 0) === profile.surpriseLevel
    && prompt.id !== excludeId
  ));
  ...
  return available[Math.floor(random() * available.length)]
    || chooseAudienceWarmupPrompt({ intensity: profile.intensity, excludeId, random });
}
