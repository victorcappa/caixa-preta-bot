...
export function selectAudienceWarmupMinigame(state, gameId, { mode = "manual", now = new Date().toISOString() } = {}) {
  ...
  if (["running", "paused", "exchange", "ready_round_two", "completed"].includes(state.status)) {
...
export function beginAudienceWarmupMinigameRound(state, now = new Date().toISOString()) {
  ...
  if (state.status === "ready_round_two" && game.id !== "tapao") {
    return { state, applied: false, reason: "MINIGAME_SECOND_ROUND_NOT_AVAILABLE" };
  }
  const round = state.status === "ready_round_two" ? 2 : 1;
  ...
  return {
    ...
    state: {
      ...
      timer: {
        status: "running",
        phase: round === 2 ? "round_two" : "round_one",
        durationSeconds,
...
export function chooseAudienceWarmupPrompt({ action, intensity, excludeId = null, recentPromptIds = [], random = Math.random } = {}) {
  let options = AUDIENCE_WARMUP_PROMPTS.filter((prompt) => (
    !prompt.requiredEverySession
    && (!action || prompt.action === action)
    && (!intensity || prompt.intensity === intensity)
    && prompt.id !== excludeId
  ));
  if (!options.length) options = AUDIENCE_WARMUP_PROMPTS.filter((prompt) => (
    !prompt.requiredEverySession && (!intensity || prompt.intensity === intensity) && prompt.id !== excludeId
  ));
  if (!options.length) options = AUDIENCE_WARMUP_PROMPTS.filter((prompt) => !prompt.requiredEverySession && prompt.id !== excludeId);
  if (!options.length) options = AUDIENCE_WARMUP_PROMPTS;
  const recentPrompts = resolveRecentPrompts(recentPromptIds);
  const seenIds = new Set(recentPromptIds.map((entry) => typeof entry === "string" ? entry : entry?.id));
  const unseenOptions = options.filter((prompt) => !seenIds.has(prompt.id));
  ...
  const ranked = options
    .map((prompt) => ({ prompt, score: audienceWarmupPromptDiversityScore(prompt, recentPromptIds) }))
    .sort((a, b) => b.score - a.score);
  const finalists = ranked.slice(0, Math.min(5, ranked.length));
  return finalists[Math.floor(random() * finalists.length)]?.prompt || null;
}
