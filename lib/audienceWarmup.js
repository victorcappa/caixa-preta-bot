...
export function createAudienceWarmupSequence({ text, action: actionId }) {
  ...
  const reactions = {
    ...
    word: ["Um.", "Dois.", "Três.", "Agora.", "Muitas respostas. Uma sala."],
  ...
  const stepTexts = [text, ...(reactions[action.id] || ["Interessante."])];
  return {
    ...
    steps: stepTexts.map((stepText, index) => ({
      id: `${action.id}-${index}`,
      text: stepText,
      kind: index === 0 ? "instruction" : index === stepTexts.length - 1 ? "reaction" : "count"
    }))
  };
}
