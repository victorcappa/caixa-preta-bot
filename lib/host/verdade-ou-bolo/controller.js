...
function autoAdvanceCommand(sequence, payload = {}) {
  return command("next", sequence, {
    delayMs: 1600,
    ...payload
  });
}
...
function moveToRound(current, index, { recovery = false } = {}) {
  ...
  const state = existingResult ? VERDADE_OU_BOLO_STATES.roundResult : VERDADE_OU_BOLO_STATES.voting;
  ...
  return {
    ...
    publicData: nextPublicData(current, {
      ...
      revealArmed: false,
      voteCountdown: existingResult ? null : votingCountdown(),
...
export function controlVerdadeOuBolo(current, action, payload = {}) {
  ...
  const autoAdvanceSequence = current.publicData.autoAdvanceCommand?.sequence || 0;
  ...
  function revealNow({ noVote = false, autoAdvance = false } = {}) {
    ...
    return {
      ...
      gameState: {
        ...current,
        phase: autoAdvance ? VERDADE_OU_BOLO_STATES.roundResult : VERDADE_OU_BOLO_STATES.reveal,
        score: { audience: score },
        publicData: nextPublicData(current, {
          state: autoAdvance ? VERDADE_OU_BOLO_STATES.roundResult : VERDADE_OU_BOLO_STATES.reveal,
          revealArmed: false,
          voteCountdown: null,
          autoAdvanceCommand: autoAdvance
            ? autoAdvanceCommand(autoAdvanceSequence, { reason: "no_vote" })
            : null,
  ...
  if (action === "reveal") {
    if (state === VERDADE_OU_BOLO_STATES.reveal && current.publicData.revealArmed) {
      return {
        applied: true,
        result: { type: "reveal_video_already_playing", round: roundIndex + 1 },
        gameState: current
      };
    }
    ...
    if (state === VERDADE_OU_BOLO_STATES.voting) {
      return {
        applied: true,
        result: { type: "vote_countdown_already_running", round: roundIndex + 1 },
        gameState: current
      };
    }
    ...
    return {
      applied: true,
      result: { type: "reveal_video_started", round: roundIndex + 1 },
      gameState: {
        ...current,
        ...
        publicData: nextPublicData(current, {
          ...
          revealedAnswer: null,
          result: null,
          videoCommand: command("restart", videoSequence)
        }),
        updatedAt: now()
      }
    };
  ...
  if (action === "vote_timeout") {
    ...
    if (!current.publicData.selectedAnswer) {
      return revealNow({ noVote: true, autoAdvance: true });
    }

    return {
      applied: true,
      result: { type: "answer_locked", selectedAnswer: current.publicData.selectedAnswer, round: roundIndex + 1 },
      gameState: {
        ...current,
        phase: VERDADE_OU_BOLO_STATES.answerLocked,
        publicData: nextPublicData(current, {
          state: VERDADE_OU_BOLO_STATES.answerLocked,
          voteCountdown: null,
          autoAdvanceCommand: null
        }),
        updatedAt: now()
