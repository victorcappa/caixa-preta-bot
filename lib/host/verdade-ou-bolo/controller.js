...
export function controlVerdadeOuBolo(current, action, payload = {}) {
  ...
  if (action === "select_answer") {
    ...
    const shouldPlayRevealVideo = Boolean(round?.video?.src);

    return {
      applied: true,
      result: {
        type: shouldPlayRevealVideo ? "answer_locked_video_started" : "answer_locked",
        selectedAnswer,
        round: roundIndex + 1
      },
      gameState: {
        ...current,
        phase: shouldPlayRevealVideo
          ? VERDADE_OU_BOLO_STATES.reveal
          : state === VERDADE_OU_BOLO_STATES.voting
            ? VERDADE_OU_BOLO_STATES.voting
            : VERDADE_OU_BOLO_STATES.answerLocked,
        publicData: nextPublicData(current, {
          state: shouldPlayRevealVideo
            ? VERDADE_OU_BOLO_STATES.reveal
            : state === VERDADE_OU_BOLO_STATES.voting
              ? VERDADE_OU_BOLO_STATES.voting
              : VERDADE_OU_BOLO_STATES.answerLocked,
          ...
          revealedAnswer: null,
          result: null,
          revealArmed: shouldPlayRevealVideo,
          voteCountdown: shouldPlayRevealVideo ? null : current.publicData.voteCountdown,
          videoCommand: shouldPlayRevealVideo
            ? command("restart", videoSequence)
            : current.publicData.videoCommand
        }),
        updatedAt: now()
      }
    };
