...
export function controlVerdadeOuBolo(current, action, payload = {}) {
  ...
  if (action === "reveal") {
    ...
    if (!round?.video?.src) {
      return {
        applied: true,
        result: { type: "vote_countdown_started", round: roundIndex + 1, missingVideo: true },
        gameState: {
          ...current,
  ...
  if (action === "vote_timeout") {
    ...
    return revealNow({
      noVote: !current.publicData.selectedAnswer,
      autoAdvance: !current.publicData.selectedAnswer
    });
  }
