...
function keywordScore(mechanic, text = "") {
  ...
  const rules = [
    ...
    [/\btime|grupo|muita gente|plateia|publico\b/, ["team_battle", "pictionary_teams", "useless_referendum", "most_likely"]],
...
function createScore(mechanic, context) {
  ...
  return Math.max(1, 12 + easyBonus + interactiveBonus + lowPriorityPenalty + statefulBonus + concreteGuessBonus + contextBonus + audienceBonus + sourceBonus + recentPenalty + cooldownPenalty);
}
...
function gameDataFor(mechanic, state, participants) {
  ...
  if (mechanic.id === "who_am_i") {
    const identities = ["algoritmo", "mala", "teatro", "notificacao", "planilha", "cancelamento", "microfone"];
...
export function selectGame(state = {}, requestedGame = null) {
  const requestedId = mapRequestedGame(requestedGame);
...
export function startGame(state = {}, { requestedGame = null, source = "operator" } = {}) {
  const selection = selectGame(state, requestedGame);
