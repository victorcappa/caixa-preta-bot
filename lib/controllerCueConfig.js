...
import { assignSceneOneSampleShortcuts } from "@/lib/sceneOneSampleShortcuts";
...
export const EDITABLE_CUE_CONTROLLERS = {
  ...
  "transicao-psicodelica": {
    ...
    cues: [
      ...
      cue("Áudio Longo", "2", "audio", "audios/CENA 2 FIM/AUDIO-2026-08-19-19-12-17.m4a", 0, "#70f4ff"),
      cue("Áudio Curto", "3", "audio", "audios/CENA 2 FIM/AUDIO-2026-08-19-19-12-26.m4a", 0, "#ff477d")
    ]
  },
  "tea-for-two": {
    id: "tea-for-two",
    title: "CENA 3 — TEA FOR TWO / TRANSIÇÃO",
    description: "Músicas, sons e textos estáticos da transição Tea For Two.",
    allowedTypes: ["audio"],
    uploadTypes: ["audio"],
    cues: [
      ...
      cue(
        ...
        "2",
        "audio",
        "audios/thomas-edson.mp3",
        0,
        "#ffffff",
        "Mesmo nesse estado, faz parte do protocolo os corpos serem fotografados e adicionados a um banco de dados digital. As fotografias possuem a finalidade de documentar a posição das vítimas e auxiliar na reconstrução do acidente. Outros métodos como identificação de impressões digitais, DNA, avaliação da arcada dentária e catalogação dos objetos pessoais farão parte desse museu online do acidente. Mesmo que você esteja morto e incapacitado de conceder ou não direitos autorais da sua imagem nesse estado, ela continuará lá nesse museu e deverá ser revisitada por inúmeros investigadores. Fique tranquilo! As imagens serão feitas por profissionais treinados. Imagens que serão acessadas por autoridades policiais e peritos. Também não são destinadas à divulgação pública. Na pior das hipóteses, vai parar no site assustador.com ou na deep web."
      )
    ]
  },
  "piloto-videogame": {
...
function normalizeControllerCueConfig(controllerId, config) {
  ...
  const rawCues = Array.isArray(config.cues) ? config.cues : [];
  const cues = controllerId === "queda-aviao-sampler"
    ? assignSceneOneSampleShortcuts(rawCues)
    : rawCues;
  ...
  return {
    ...
    cues: cues.map((item) => normalizeCue(item, allowedTypes)).filter(Boolean)
  };
}
...
function normalizeCue(item, allowedTypes) {
  ...
  const requestedShortcut = `${item.shortcut || ""}`.trim().slice(0, 24);

  return {
    id: `${item.id || crypto.randomUUID()}`,
    label: `${item.label || "Novo Botão"}`.trim() || "Novo Botão",
    shortcut: requestedShortcut.toLowerCase() === "l" ? "" : requestedShortcut,
    type,
