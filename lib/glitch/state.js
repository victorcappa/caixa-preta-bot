...
export const GLITCH_AUDIO_PRESETS = {
  normal: { typingIntensity: 0.34, ghostTyping: false },
  strong: { typingIntensity: 0.86, ghostTyping: true },
  continuous: { typingIntensity: 0.64, ghostTyping: false },
  video: { typingIntensity: 0.92, ghostTyping: true }
};

export function glitchAudioForPreset(preset = "normal", overrides = {}) {
  const base = GLITCH_AUDIO_PRESETS[preset] || GLITCH_AUDIO_PRESETS.normal;
  return {
    typingIntensity: clampNumber(overrides.typingIntensity, 0, 1, base.typingIntensity),
    ghostTyping: overrides.ghostTyping === undefined ? base.ghostTyping : Boolean(overrides.ghostTyping)
  };
}
...
export function createInitialGlitchState() {
  return {
    ...
    audio: glitchAudioForPreset("normal"),
    video: {
      active: false,
      src: null,
      file: null,
...
export function publicGlitchSnapshot(glitch = createInitialGlitchState()) {
  return {
    ...
    audio: glitchAudioForPreset(glitch.preset, glitch.audio),
    video: { ...(glitch.video || {}) }
  };
}
