...
const DEFAULTS_PATH = path.join(process.cwd(), "data", "queda-aviao-default.json");
const DEFAULT_KEYS = ["rawText", "subdivision", "fadeMs", "pace", "stageMultiplier", "loop"];

export function createInitialQuedaAviaoState() {
  const now = new Date().toISOString();
  const defaults = readQuedaAviaoDefaults();
...
export function saveQuedaAviaoDefaults(state) {
  const normalized = normalizeQuedaAviaoState(state);
  const defaults = Object.fromEntries(DEFAULT_KEYS.map((key) => [key, normalized[key]]));
...
function readQuedaAviaoDefaults() {
  try {
    const parsed = JSON.parse(fs.readFileSync(DEFAULTS_PATH, "utf8"));
    return DEFAULT_KEYS.reduce((defaults, key) => {
