...
const SYSTEM95_STYLE = "system95";
const SYSTEM95_KEY_NOTES = [392, 440, 523.25, 587.33, 659.25, 783.99];
const ATTENTION_REPEAT_DELAY_MS = 520;
...
export class RobotSoundEngine {
  ...
  armAudioRelay({ sink = false } = {}) {
    ...
    return () => {
      ...
      if (sink) {
        this.relaySinkUsers = Math.max(0, this.relaySinkUsers - 1);
        if (this.relaySinkUsers === 0) {
          this.relayChannel?.postMessage({ type: "sink-offline", sinkId: this.relaySinkId });
        }
      }
  ...
  handleRelayMessage(message = {}) {
    ...
    if (message.type === "sink") {
      if (!message.sinkId) return;
      this.relaySinks.set(message.sinkId, Date.now());
      this.lastRelaySinkAt = Date.now();
      return;
    }
    if (message.type === "sink-offline") {
      this.relaySinks.delete(message.sinkId);
      return;
    }
    if (
      message.type !== "play" ||
      message.targetSinkId !== this.relaySinkId ||
      this.relaySinkUsers < 1 ||
    ...
    if (message.effect === "attention") this.attention({ localOnly: true });
    ...
    if (message.effect === "roulette-tick") this.rouletteTick(message.step, { localOnly: true });
    if (message.effect === "game-start") this.gameStart({ localOnly: true });
    if (message.effect === "game-over") this.gameOver({ localOnly: true });
    if (message.effect === "whistle") this.whistle({ localOnly: true });
    if (message.effect === "hangman-pulse") this.hangmanPulse(message.step, { localOnly: true });
    if (message.effect === "hangman-stop") this.stopHangmanMusic({ localOnly: true });
    if (message.effect === "stop-all") this.stopAll({ localOnly: true });
  }

  relayEffect(effect, detail = {}) {
    if (!this.relayChannel) return false;
    const now = Date.now();
    for (const [sinkId, seenAt] of this.relaySinks) {
      if (now - seenAt > AUDIO_RELAY_TTL_MS) this.relaySinks.delete(sinkId);
    }
    const targetSinkId = [...this.relaySinks.keys()].sort()[0];
    if (!targetSinkId) return false;
    this.relayChannel.postMessage({ type: "play", targetSinkId, effect, ...detail });
    return true;
  }
  ...
  usesSystem95Style() {
    return this.settings.soundStyle === SYSTEM95_STYLE;
  }
  ...
  typing(character, { force = false, localOnly = false } = {}) {
    ...
    if (this.usesSystem95Style()) {
      const characterCode = `${character || " "}`.codePointAt(0) || 0;
      const keyPitch = SYSTEM95_KEY_NOTES[characterCode % SYSTEM95_KEY_NOTES.length];
      const systemShape = {
        key: { pitch: keyPitch, level: 0.16, durationMs: 34 },
        space: { pitch: 329.63, level: 0.045, durationMs: 18 },
        pause: { pitch: 523.25, level: 0.13, durationMs: 42 },
        period: { pitch: 392, level: 0.18, durationMs: 54 },
        emphasis: { pitch: 783.99, level: 0.18, durationMs: 48 },
        return: { pitch: 293.66, level: 0.2, durationMs: 62 }
      }[kind];
      const pitchDirection = characterCode % 2 === 0 ? 0.75 : 1.25;
      this.computerBlip({
        pitch: systemShape.pitch * glitchPitch,
        toPitch: systemShape.pitch * pitchDirection * glitchPitch,
        durationMs: systemShape.durationMs,
        level: systemShape.level,
  ...
  click(options) {
    ...
    const noiseAmount = Math.max(0, Number(options.noise) || 0);
    const destination = options.destination || this.effectsGain;

    oscillator.type = "square";
    oscillator.frequency.value = options.pitch * this.settings.pitchScale * randomBetween(1 - options.pitchVariation, 1 + options.pitchVariation);
    ...
    if (noiseAmount > 0) {
      ...
      noiseGain.gain.exponentialRampToValueAtTime(clamp(level * noiseAmount, 0.0002, 0.58), start + attack * 0.7);
  ...
  computerBlip({ pitch, toPitch = pitch, durationMs = 34, level = 0.16, destination = this.typingGain }) {
    if (!this.canPlay()) return;
    const context = this.context;
    const start = context.currentTime + 0.001;
    const end = start + clamp(durationMs / 1000, 0.014, 0.08);
    const step = start + Math.min(0.018, (end - start) * 0.52);
    const oscillator = context.createOscillator();
    const filter = context.createBiquadFilter();
    const gain = context.createGain();
    const pitchScale = this.settings.pitchScale;

    oscillator.type = "square";
    oscillator.frequency.setValueAtTime(pitch * pitchScale, start);
    oscillator.frequency.setValueAtTime(toPitch * pitchScale, step);
    filter.type = "lowpass";
    filter.frequency.value = 1900 * Math.min(1.25, pitchScale);
    filter.Q.value = 0.7;
    gain.gain.setValueAtTime(0.0001, start);
    gain.gain.linearRampToValueAtTime(level, start + 0.0015);
    gain.gain.setValueAtTime(level, Math.max(start + 0.0015, end - 0.005));
  ...
  tone({ from, to = from, durationMs, level = 0.1, type = "sine" }) {
    ...
    oscillator.frequency.setValueAtTime(from * this.settings.pitchScale, start);
    oscillator.frequency.exponentialRampToValueAtTime(Math.max(20, to * this.settings.pitchScale), end);
    gain.gain.setValueAtTime(0.0001, start);
  ...
  system95Sequence(notes, { gapMs = 48, kind = "system95" } = {}) {
    for (const [index, note] of notes.entries()) {
      const play = () => this.tone({
        from: note.frequency,
        durationMs: note.durationMs || 42,
        level: note.level || 0.08,
        type: note.type || "square"
      });
      if (index === 0) play();
      else this.scheduleTimer(play, Number(note.delayMs) || index * gapMs, kind);
    }
  }
  ...
  wake({ localOnly = false } = {}) {
    ...
    if (this.usesSystem95Style()) {
      this.system95Sequence([
        { frequency: 523.25, durationMs: 38, level: 0.075 },
        { frequency: 659.25, durationMs: 38, level: 0.075 },
        { frequency: 783.99, durationMs: 55, level: 0.09 }
      ], { gapMs: 44, kind: "wake" });
      return;
    }
  ...
  playAttentionPhrase() {
    this.playSampleLayer("attention");
    if (this.usesSystem95Style()) {
      this.system95Sequence([
        { frequency: 880, durationMs: 72, level: 0.13 },
        { frequency: 1174.66, durationMs: 92, level: 0.15 },
        { frequency: 880, durationMs: 72, level: 0.13 },
        { frequency: 1174.66, durationMs: 120, level: 0.17 }
      ], { gapMs: 92, kind: "attention" });
      return;
    }
    this.tone({ from: 520, to: 920, durationMs: 110, level: 0.12, type: "square" });
    this.scheduleTimer(() => {
      this.tone({ from: 620, to: 1120, durationMs: 140, level: 0.14, type: "square" });
    }, 150, "attention");
  }

  attention({ localOnly = false } = {}) {
    if (!localOnly && this.relayEffect("attention")) return;
    this.playAttentionPhrase();
    this.scheduleTimer(() => this.playAttentionPhrase(), ATTENTION_REPEAT_DELAY_MS, "attention");
  }
  ...
  complete({ localOnly = false } = {}) {
    ...
    if (this.usesSystem95Style()) {
      this.system95Sequence([
        { frequency: 783.99, durationMs: 28, level: 0.055 },
        { frequency: 1046.5, durationMs: 42, level: 0.065 }
      ], { gapMs: 32, kind: "complete" });
      return;
    }
  ...
  countdown(value, { localOnly = false } = {}) {
    ...
    if (this.usesSystem95Style()) {
      this.playSampleLayer(seconds === 0 ? "countdownComplete" : "countdown", {
        value: seconds,
        urgency: seconds <= 3
      });
      if (seconds === 0) {
        this.system95Sequence([
          { frequency: 392, durationMs: 42, level: 0.1 },
          { frequency: 523.25, durationMs: 42, level: 0.1 },
          { frequency: 783.99, durationMs: 72, level: 0.12 }
        ], { gapMs: 46, kind: "countdown" });
      } else {
        const frequency = seconds <= 3 ? 880 + ((3 - seconds) * 146.5) : 659.25;
        this.click({ pitch: frequency, pitchVariation: 0, durationMs: 34, attackMs: 0.6, decayMs: 31, noise: 0, filterHz: 4200, level: seconds <= 3 ? 0.22 : 0.15 });
      }
      return;
    }
  ...
  success({ localOnly = false } = {}) {
    ...
    if (this.usesSystem95Style()) {
      this.system95Sequence([
        { frequency: 523.25, durationMs: 34, level: 0.085 },
        { frequency: 659.25, durationMs: 34, level: 0.085 },
        { frequency: 783.99, durationMs: 34, level: 0.09 },
        { frequency: 1046.5, durationMs: 70, level: 0.11 }
      ], { gapMs: 38, kind: "success" });
      return;
    }
  ...
  error({ localOnly = false } = {}) {
    ...
    if (this.usesSystem95Style()) {
      this.system95Sequence([
        { frequency: 246.94, durationMs: 70, level: 0.12 },
        { frequency: 185, durationMs: 90, level: 0.13 }
      ], { gapMs: 76, kind: "error" });
      return;
    }
  ...
  glitchEffect({ localOnly = false, intensity = this.glitch.intensity } = {}) {
    ...
    if (this.usesSystem95Style()) {
      const systemNotes = [1318.51, 659.25, 987.77, 329.63, 1174.66];
      for (let index = 0; index < 2 + Math.round(normalizedIntensity * 2); index += 1) {
        this.scheduleTimer(() => this.click({
          pitch: systemNotes[index % systemNotes.length], pitchVariation: 0, durationMs: 16,
          attackMs: 0.4, decayMs: 13, noise: 0, filterHz: 5200,
          level: 0.11 + normalizedIntensity * 0.08
        }), index * 22, "glitch");
      }
      return;
    }
  ...
  impact({ localOnly = false } = {}) {
    ...
    if (this.usesSystem95Style()) {
      this.system95Sequence([
        { frequency: 110, durationMs: 62, level: 0.13, type: "square" },
        { frequency: 82.41, durationMs: 110, level: 0.14, type: "square" }
      ], { gapMs: 58, kind: "impact" });
      return;
    }
  ...
  rouletteTick(step = 0, { localOnly = false } = {}) {
    const index = Math.max(0, Math.round(Number(step) || 0));
    if (!localOnly && this.relayEffect("roulette-tick", { step: index })) return;
    if (!this.canPlay()) return;
    const pitches = [523.25, 659.25, 783.99, 880];
  ...
  gameStart({ localOnly = false } = {}) {
    if (!localOnly && this.relayEffect("game-start")) return;
    this.playSampleLayer("gameStart");
    this.system95Sequence([
      { frequency: 392, durationMs: 44, level: 0.075 },
      { frequency: 523.25, durationMs: 44, level: 0.085 },
      { frequency: 783.99, durationMs: 76, level: 0.1 }
    ], { gapMs: 52, kind: "game-start" });
  }

  gameOver({ localOnly = false } = {}) {
    if (!localOnly && this.relayEffect("game-over")) return;
    this.playSampleLayer("gameOver");
    this.system95Sequence([
      { frequency: 659.25, durationMs: 72, level: 0.09 },
      { frequency: 493.88, durationMs: 82, level: 0.095 },
      { frequency: 369.99, durationMs: 105, level: 0.1 },
      { frequency: 196, durationMs: 180, level: 0.11 }
    ], { gapMs: 94, kind: "game-over" });
  }

  whistle({ localOnly = false } = {}) {
    if (!localOnly && this.relayEffect("whistle")) return;
    this.playSampleLayer("whistle");
    this.system95Sequence([
      { frequency: 1760, durationMs: 210, level: 0.105, type: "sine" },
      { frequency: 1864.66, durationMs: 330, level: 0.12, type: "sine", delayMs: 285 }
    ], { kind: "whistle" });
  }

  hangmanPulse(step = 0, { localOnly = false } = {}) {
    const index = Math.max(0, Math.round(Number(step) || 0));
    if (!localOnly && this.relayEffect("hangman-pulse", { step: index })) return;
    if (!this.canPlay()) return;
    const motif = [164.81, 196, 146.83, 220];
    const root = motif[index % motif.length];
    this.tone({ from: root, to: root * 0.985, durationMs: 260, level: 0.034, type: "square" });
    this.scheduleTimer(() => {
      this.tone({ from: root * 1.5, to: root * 1.48, durationMs: 150, level: 0.022, type: "triangle" });
    }, 150, "hangman-music");
  }

  stopHangmanMusic({ localOnly = false } = {}) {
    if (!localOnly) this.relayEffect("hangman-stop");
    for (const timer of [...this.timers]) {
      if (timer.kind !== "hangman-music") continue;
      window.clearTimeout(timer.id);
      this.timers.delete(timer);
    }
  }
  ...
  startThinking({ localOnly = false } = {}) {
    ...
    const pulse = () => {
      ...
      if (this.canPlay()) {
        if (this.usesSystem95Style()) {
          const pitch = Math.random() < 0.5 ? 523.25 : 659.25;
          this.click({ pitch, pitchVariation: 0, durationMs: 18, attackMs: 0.4, decayMs: 15, noise: 0, filterHz: 4200, level: 0.065 });
        } else {
      ...
      this.scheduleTimer(pulse, this.usesSystem95Style() ? randomBetween(420, 920) : randomBetween(520, 1450), "thinking");
    };
...
export const robotSoundEngine = globalThis[globalKey]?.attention
  ? globalThis[globalKey]
  : new RobotSoundEngine();
