...
export function buildInteractionHistoryBlock(conversation = []) {
  ...
  const lastWasClosure = lastAssistant
    ...
    : false;
  const conversationalTurns = recentAssistantTurns.filter((turn) => (
    CONVERSATIONAL_PATTERNS.some((pattern) => pattern.test(turn.excerpt)) ||
    !TASKY_PATTERNS.some((pattern) => pattern.test(turn.excerpt))
  )).length;
  const taskyTurns = recentAssistantTurns.filter((turn) => (
    TASKY_PATTERNS.some((pattern) => pattern.test(turn.excerpt))
  )).length;
  const conversationRunActive = conversationalTurns >= 2 && taskyTurns <= 1;
...
conversationRunActive = ${conversationRunActive}
conversationRunTurns = ${conversationalTurns}
recentTaskyTurns = ${taskyTurns}
...
Varie entre comentario, follow-up genuino, fofoca, consequencia, acao fisica,
callback, escolha, voto, roast leve, regra arbitraria, outra pessoa, pausa ou
comando direto.
Se conversationRunActive = true e a conversa esta interessante, continue
conversando. Nao force tarefa so para preencher o turno.
