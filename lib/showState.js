...
function createStore() {
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "select-action") {
      ...
      state.audienceWarmup = { ...current, preview: prompt.text, previewPromptId: prompt.id, updatedAt: now };
      message = "PERGUNTA GERADA — CONFIRA O PREVIEW";
    } else if (action === "surprise") {
      const prompt = chooseAudienceWarmupPrompt({ excludeId: current.previewPromptId });
      state.audienceWarmup = { ...current, selectedAction: prompt.action, intensity: prompt.intensity, preview: prompt.text, previewPromptId: prompt.id, updatedAt: now };
      message = "COMBINAÇÃO SURPRESA GERADA — CONFIRA O PREVIEW";
