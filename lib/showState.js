...
function createStore() {
  ...
  function publishAudienceWarmupStep(stepIndex, { repeated = false } = {}) {
    ...
    state.audienceWarmup = {
      ...
      awaitingOperator: !state.audienceWarmup.automatic,
      display: {
    ...
    if (state.audienceWarmup.automatic && sequence.steps[stepIndex + 1]) {
      ...
      }, state.audienceWarmup.intervalMs);
    } else if (!sequence.steps[stepIndex + 1]) {
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "select-action") {
      ...
      clearAudienceWarmupTimer();
      const sequence = createAudienceWarmupSequence({ text: prompt.text, action: current.selectedAction });
      ...
      clearAudienceWarmupTimer();
      const sequence = createAudienceWarmupSequence({ text: prompt.text, action: prompt.action });
