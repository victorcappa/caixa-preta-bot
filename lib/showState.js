...
function createStore() {
  ...
  function schedulePlayUnlockBoot() {
    ...
    playUnlockTimer = setTimeout(() => {
      ...
      if (!advanced.advanced) {
        ...
        scheduleHumanVerificationIntro();
        return;
      }
  ...
  function scheduleHumanVerificationIntro() {
    if (playUnlockTimer) clearTimeout(playUnlockTimer);
    playUnlockTimer = setTimeout(() => {
      playUnlockTimer = null;
      if (state.sceneZero.unlock.status !== PLAY_UNLOCK_STATES.BOOT_FAILED) return;
    ...
    }, PLAY_UNLOCK_CONFIG.bootFailureDurationMs);
  }
  ...
  function controlPlayUnlock(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "boot") {
      ...
      if (current.status === PLAY_UNLOCK_STATES.STANDBY) return { ok: false, error: "BIOS NOT STARTED", state: { ...current } };
