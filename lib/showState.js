...
const PROJECTION_HEARTBEAT_TIMEOUT_MS = 15000;
...
function createStore() {
  ...
  function registerProjectionWindow({ id, path, userAgent = null } = {}) {
    ...
    if (!projectionWindowId) {
      return { applied: false, error: "PROJECTION WINDOW ID REQUIRED", state: publicProjectionSnapshot(state.projection) };
    }
...
export const showState = globalThis[globalKey]?.startGame && globalThis[globalKey]?.controlStructuredGame && globalThis[globalKey]?.cancelPerformanceEvents && globalThis[globalKey]?.startSuitcases && globalThis[globalKey]?.updateInstagram && globalThis[globalKey]?.drawBaralhoMorbidoCard && globalThis[globalKey]?.controlQuedaAviao && globalThis[globalKey]?.controlGlitch && globalThis[globalKey]?.controlDisplayBlackout && globalThis[globalKey]?.registerProjectionWindow
