...
function createStore() {
  ...
  function schedulePlayUnlockBoot() {
    ...
    playUnlockTimer = setTimeout(() => {
      ...
      if (!advanced.advanced) {
        ...
        if (state.sceneZero.suitcaseGame?.status === "finished") {
  ...
  function animatePlayUnlockTo100(source = "suitcases-finished") {
    clearPlayUnlockTimers();
    ...
    state.sceneZero.unlock = {
      ...
      lastProgressAction: "fim do Jogo das Malas",
      lastIncreaseAction: "fim do Jogo das Malas",
      updatedAt: new Date().toISOString()
    };
  ...
  function controlPlayUnlock(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "boot") {
      ...
      const result = adjustPlayUnlockProgress(current, direction * amount, {
        label: payload.label || (direction > 0 ? "ação aumentou o carregamento" : "ação diminuiu o carregamento")
      });
      if (!result.applied) return { ok: false, error: result.reason, state: { ...current } };
      state.sceneZero.unlock = result.state;
      appendSceneZeroAction("play-unlock-progress", `${direction > 0 ? "+" : "-"}${amount}% / ${state.sceneZero.unlock.progress}%`);
      emitPlayUnlock("play-unlock-progress");
      message = `CARREGAMENTO ${direction > 0 ? "+" : "-"}${amount}%`;
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    const requiresBoot = ["generate", "surprise", "send", "next", "repeat"].includes(action);
    if (requiresBoot && [PLAY_UNLOCK_STATES.STANDBY, PLAY_UNLOCK_STATES.BOOTING].includes(state.sceneZero.unlock.status)) {
