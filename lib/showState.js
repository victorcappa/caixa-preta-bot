...
function createStore() {
  ...
  function schedulePlayUnlockBoot() {
    ...
    playUnlockTimer = setTimeout(() => {
      ...
      if (!advanced.advanced) {
        ...
        addMessage("assistant", PLAY_UNLOCK_CONFIG.openingLine, "scene-zero-unlock");
        emitPlayUnlock("play-unlock-waiting");
        return;
      }
  ...
  function playUnlockFeedback() {
    const nextScoredCount = (state.sceneZero.unlock.scoredActionIds?.length || 0) + 1;
    if (nextScoredCount % PLAY_UNLOCK_CONFIG.feedbackEvery !== 0) return null;
    const index = Math.floor(nextScoredCount / PLAY_UNLOCK_CONFIG.feedbackEvery - 1)
      % PLAY_UNLOCK_CONFIG.technicalFeedback.length;
    return PLAY_UNLOCK_CONFIG.technicalFeedback[index];
  }

  function preparePlayUnlockForWarmup() {
    if (state.sceneZero.unlock.status !== PLAY_UNLOCK_STATES.BOOTING) return;
    if (playUnlockTimer) clearTimeout(playUnlockTimer);
    playUnlockTimer = null;
    state.sceneZero.unlock = stallPlayUnlockBoot({
      ...state.sceneZero.unlock,
      bootStep: PLAY_UNLOCK_CONFIG.bootSteps.length - 1
    });
    appendSceneZeroAction("play-unlock-warmup-interrupt", `${PLAY_UNLOCK_CONFIG.bootLimit}%`);
    emitPlayUnlock("play-unlock-waiting");
  }

  function scoreAudienceWarmupSequence(sequence, { source = "operator" } = {}) {
    const result = scorePlayUnlockAction(state.sceneZero.unlock, {
      actionId: sequence.progressId,
      ...
      feedback: playUnlockFeedback(),
      externalInput: { source, type: "operator-confirmed-action", automaticDetection: false }
    });
    if (!result.applied && result.reason === "ZERO_PROGRESS") {
      state.sceneZero.unlock = {
        ...result.state,
        status: PLAY_UNLOCK_STATES.WARMING_AUDIENCE,
        lastProgressAction: `${sequence.steps?.[0]?.text || sequence.label} / sem progressão`,
        externalInput: { source, type: "operator-confirmed-action", automaticDetection: false },
        updatedAt: new Date().toISOString()
      };
      emitPlayUnlock("play-unlock-warming");
      return { ...result, state: state.sceneZero.unlock };
    }
    if (!result.applied) return result;
    state.sceneZero.unlock = result.state;
    appendSceneZeroAction("play-unlock-progress", `+${sequence.progressValue}% / ${sequence.progressId}`);
    if (result.reached100) schedulePlayUnlockSequence("audience-warmup");
    else emitPlayUnlock("play-unlock-progress");
    return result;
  }
  ...
  function animatePlayUnlockTo100(source = "operator-complete") {
    ...
    state.sceneZero.unlock = {
      ...
      lastProgressAction: "completar barra",
      lastIncreaseAction: "completar barra",
      updatedAt: new Date().toISOString()
    };
  ...
  function controlPlayUnlock(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "pause-boot") {
    ...
    } else if (action === "increment" || action === "external-input") {
      const amount = Math.max(0, Number(payload.amount) || PLAY_UNLOCK_CONFIG.manualProgressStep);
      const result = scorePlayUnlockAction(current, {
        actionId: null,
        label: payload.label || (action === "external-input" ? "entrada externa" : "participação manual"),
        progressValue: amount,
        repeatableProgress: true,
        feedback: PLAY_UNLOCK_CONFIG.technicalFeedback[current.feedbackSequence % PLAY_UNLOCK_CONFIG.technicalFeedback.length],
        externalInput: {
          source: action === "external-input" ? source : "operator",
          type: payload.type || "manual",
          automaticDetection: action === "external-input"
        }
      });
      if (!result.applied) return { ok: false, error: result.reason, state: { ...current } };
      state.sceneZero.unlock = result.state;
      if (result.reached100) schedulePlayUnlockSequence(action);
      else emitPlayUnlock("play-unlock-progress");
      message = `PARTICIPAÇÃO +${amount}%`;
    } else if (action === "decrement") {
      const amount = Math.max(0, Number(payload.amount) || PLAY_UNLOCK_CONFIG.manualProgressStep);
      state.sceneZero.unlock = setPlayUnlockProgress(current, current.progress - amount, { label: "redução manual" });
      emitPlayUnlock("play-unlock-progress");
      message = `PARTICIPAÇÃO -${amount}%`;
    } else if (action === "set-progress") {
      state.sceneZero.unlock = setPlayUnlockProgress(current, payload.progress, { label: "progresso definido pelo operador" });
      if (state.sceneZero.unlock.progress === 100) schedulePlayUnlockSequence("operator-set-progress");
      else emitPlayUnlock("play-unlock-progress");
      message = `PROGRESSO DEFINIDO: ${state.sceneZero.unlock.progress}%`;
    } else if (action === "complete") {
      animatePlayUnlockTo100("operator-complete");
      message = "COMPLETANDO BARRA";
    } else if (action === "force") {
      finishPlayUnlock("operator-force");
      message = "PEÇA DESBLOQUEADA AGORA";
      ...
      schedulePlayUnlockBoot();
      message = "DESBLOQUEIO REINICIADO";
    } else {
  ...
  function publishAudienceWarmupStep(stepIndex, { repeated = false } = {}) {
    ...
    if (step.kind === "instruction" && !repeated) {
      scoreAudienceWarmupSequence(sequence);
    }
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "select-action") {
      ...
      clearAudienceWarmupTimer();
      preparePlayUnlockForWarmup();
      const sequence = createAudienceWarmupSequence({ text, action: current.selectedAction });
