...
import { AUDIENCE_WARMUP_PROMPTS, AUDIENCE_WARMUP_REQUIRED_PROMPT_ID } from "@/data/audience-warmup-prompts";
import {
...
const STORE_VERSION = "scene-zero-fixed-timers-and-intro-v6";
...
function createStore() {
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "set-manual-mode") {
      ...
      audienceWarmupTimer = setTimeout(() => {
        state.audienceWarmup = { ...state.audienceWarmup, active: false, actionTimer: { ...state.audienceWarmup.actionTimer, status: "complete", remainingSeconds: 0, endsAt: null }, updatedAt: new Date().toISOString() };
        emitAudienceWarmup();
      }, durationSeconds * 1000);
