...
import { PUBLIC_TYPE_INTERVAL_MS, sequentialMessageSchedule } from "@/lib/messageTiming";
import {
...
const STORE_VERSION = "scene-zero-suitcase-games-v5";
...
function createStore() {
  ...
  function schedulePlayUnlockBoot() {
    ...
    playUnlockTimer = setTimeout(() => {
      ...
      if (!advanced.advanced) {
        ...
        appendSceneZeroAction("play-unlock-waiting", `${PLAY_UNLOCK_CONFIG.bootLimit}%`);
  ...
  function publishAudienceWarmupSystemSequence(lines, onComplete, stepIndex = 0) {
    ...
    const readingMs = Math.max(AUDIENCE_WARMUP_MINIGAME_STEP_DURATION_MS, text.length * PUBLIC_TYPE_INTERVAL_MS + 650);
    audienceWarmupTimer = setTimeout(() => {
      audienceWarmupTimer = null;
      if (stepIndex + 1 >= lines.length) onComplete?.();
      else publishAudienceWarmupSystemSequence(lines, onComplete, stepIndex + 1);
    }, readingMs);
  }
  ...
  function startAudienceWarmupBriefing() {
    ...
    publishAudienceWarmupSystemSequence(AUDIENCE_WARMUP_MINIGAME_INTRO, () => {
      state.audienceWarmup = {
        ...state.audienceWarmup,
        active: false,
        phase: "minigame",
  ...
  function startHumanVerificationIntro() {
    ...
    playUnlockTimer = setTimeout(() => {
      playUnlockTimer = null;
      if (state.sceneZero.unlock.status !== PLAY_UNLOCK_STATES.HUMAN_VERIFICATION) return;
    ...
    }, PLAY_UNLOCK_CONFIG.verificationTitleDurationMs);
    return true;
  }
  ...
  function recordAudienceWarmupSequence(sequence, { source = "operator" } = {}) {
    const result = recordPlayUnlockAudienceAction(state.sceneZero.unlock, {
      ...
      externalInput: { source, type: "operator-confirmed-action", sequenceId: sequence.id, automaticDetection: false }
    });
    if (!result.applied) return result;
    state.sceneZero.unlock = result.state;
    appendSceneZeroAction("play-unlock-warmup", `${sequence.progressId} / aguardando avaliação`);
  ...
  function queuePlayUnlockProgress(delta, options = {}) {
    ...
    playUnlockTimer = setTimeout(() => {
      ...
      if (state.sceneZero.unlock.progress >= 100) schedulePlayUnlockSequence(options.source || "human-verification");
    }, PLAY_UNLOCK_CONFIG.feedbackLeadMs);
    return prepared;
  }

  function animatePlayUnlockTo100(source = "operator-complete") {
    clearPlayUnlockTimers();
    const showCompletedBarThenSequence = () => {
      if (source !== "suitcases-finished") {
    ...
    playUnlockAnimationTimer = setInterval(() => {
      ...
      state.sceneZero.unlock = {
        ...state.sceneZero.unlock,
        progress: Math.round(from + (100 - from) * eased),
        updatedAt: new Date().toISOString()
      };
      if (ratio >= 1) {
  ...
  function controlPlayUnlock(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "boot") {
      ...
      if (progress >= 100) schedulePlayUnlockSequence("operator-set-progress");
      message = `PROGRESSO DEFINIDO: ${progress}%`;
    } else if (action === "complete") {
      if ([PLAY_UNLOCK_STATES.STANDBY, PLAY_UNLOCK_STATES.BOOTING, PLAY_UNLOCK_STATES.BOOT_FAILED, PLAY_UNLOCK_STATES.HUMAN_VERIFICATION].includes(current.status)) {
        return { ok: false, error: "HUMAN VERIFICATION NOT READY", state: { ...current } };
      }
      animatePlayUnlockTo100("operator-complete");
      message = "BARRA EM CONCLUSÃO";
    } else if (action === "unlock-now") {
      if (![PLAY_UNLOCK_STATES.WAITING_FOR_AUDIENCE, PLAY_UNLOCK_STATES.WARMING_AUDIENCE].includes(current.status)) {
        return { ok: false, error: "HUMAN VERIFICATION NOT READY", state: { ...current } };
      }
      schedulePlayUnlockSequence("operator-force");
      message = "DESBLOQUEIO IMEDIATO INICIADO";
  ...
  function publishAudienceWarmupStep(stepIndex, { repeated = false } = {}) {
    ...
    const shouldAutoAdvance = Boolean(nextStep && (state.audienceWarmup.automatic || countdownMustContinue));
    state.audienceWarmup = {
      ...state.audienceWarmup,
      active: true,
    ...
    if (step.kind === "instruction" && !repeated) {
      recordAudienceWarmupSequence(sequence);
    }
    ...
    if (shouldAutoAdvance) {
      ...
      audienceWarmupTimer = setTimeout(() => {
        publishAudienceWarmupStep(stepIndex + 1);
        emit({ type: "audience-warmup", audienceWarmup: publicAudienceWarmupSnapshot(state.audienceWarmup) });
      }, delayMs);
  ...
  function drawAudienceWarmupSelection(current, { selectedAction = null, intensity = null, progressive = false } = {}, now = new Date().toISOString()) {
    ...
    const surpriseCount = progressive ? Number(current.surpriseCount || 0) + 1 : Number(current.surpriseCount || 0);
    const prompt = progressive
      ? chooseAudienceWarmupSurprisePrompt({
        count: surpriseCount,
  ...
  function presentAudienceWarmupMinigameRules() {
    ...
    publishAudienceWarmupSystemSequence(rules, () => {
      const currentMinigame = state.audienceWarmup.minigame;
      if (currentMinigame.status !== "instructions" || currentMinigame.selectedId !== game.id) return;
      state.audienceWarmup = {
        ...state.audienceWarmup,
        active: false,
        minigame: { ...currentMinigame, status: "ready", round: 1, updatedAt: new Date().toISOString() },
        updatedAt: new Date().toISOString()
      };
      emitAudienceWarmup();
    });
  ...
  function drawAudienceWarmupMinigame() {
    ...
    audienceWarmupTimer = setTimeout(() => {
      audienceWarmupTimer = null;
      if (state.audienceWarmup.minigame.status !== "drawing") return;
      revealAudienceWarmupMinigame(game.id, "random");
    }, AUDIENCE_WARMUP_MINIGAME_DRAW_DURATION_MS);
  ...
  function scheduleAudienceWarmupMinigameExpiry() {
    ...
    audienceWarmupTimer = setTimeout(() => {
      audienceWarmupTimer = null;
      const active = state.audienceWarmup.minigame;
      if (active.timer?.status !== "running") return;
      if (active.timer.phase === "countdown") {
        const begun = beginAudienceWarmupMinigameRound(active);
        if (!begun.applied) return;
      ...
      } else if (active.timer.phase === "exchange") {
        state.audienceWarmup = {
          ...state.audienceWarmup,
          active: false,
          minigame: {
            ...
            round: 2,
            timer: { ...active.timer, status: "idle", remainingSeconds: 0, endsAt: null },
            updatedAt: new Date().toISOString()
          },
          updatedAt: new Date().toISOString()
        };
        publishAudienceWarmupSystemText("SEGUNDO TURNO PRONTO.");
      } else if (active.selectedId === "tapao" && active.round === 1) {
        beginAudienceWarmupTapaoExchange();
      } else {
        finishAudienceWarmupMinigame();
      }
    }, delayMs);
  }
  ...
  function launchAudienceWarmupProgressiveQuestion() {
    const current = state.audienceWarmup;
    const surpriseCount = Number(current.surpriseCount || 0) + 1;
    const prompt = chooseAudienceWarmupSurprisePrompt({
      count: surpriseCount,
      excludeId: current.previewPromptId,
      recentPromptIds: current.recentPromptIds
    });
  ...
  function finishAudienceWarmupMinigame({ skipped = false } = {}) {
    ...
    const transition = skipped
      ? ["MINIGAME IGNORADO.", "AGORA ALGUMAS PERGUNTAS."]
      : ["FIM.", "TESTE MOTOR CONCLUÍDO.", "AGORA ALGUMAS PERGUNTAS."];
    publishAudienceWarmupSystemSequence(transition, launchAudienceWarmupProgressiveQuestion);
    return { applied: true, state: state.audienceWarmup.minigame };
  }
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (questionActions.includes(action) && current.phase !== "questions") {
      return { ok: false, error: "CONCLUA OU PULE O MINIGAME ANTES DAS PERGUNTAS", state: publicAudienceWarmupSnapshot(current) };
    }
    ...
    if (action === "minigame-draw") {
      ...
      if (current.phase === "questions" || current.minigame.status === "completed") return { ok: false, error: "MINIGAME_ALREADY_FINISHED", state: publicAudienceWarmupSnapshot(current) };
      ...
      const prompt = drawAudienceWarmupSelection(current, { progressive: true }, now);
      message = `SORTEADO DA BIBLIOTECA: ${prompt.id}`;
      ...
      publishAudienceWarmupStep(0);
      if (payload.withProgress && prompt.progressValue > 0) {
        const score = queuePlayUnlockProgress(prompt.progressValue, {
          label: prompt.text,
          feedbackOverride: prompt.feedbackOverride,
          source: "human-verification"
        });
        if (!score.applied && score.reason !== "ACTION_ALREADY_EVALUATED") {
          return { ok: false, error: score.reason, state: publicAudienceWarmupSnapshot(state.audienceWarmup) };
        }
        message = score.applied
          ? `AÇÃO DISPARADA · +${prompt.progressValue}%`
          : "AÇÃO DISPARADA · PROGRESSO JÁ CONTABILIZADO";
      } else {
        message = "AÇÃO DISPARADA SEM PROGRESSO";
      }
    } else if (action === "next") {
  ...
  function applySuitcaseResult(result, { source = "system", queueEvents = true } = {}) {
    ...
    if (result.result?.type === "suitcases_finished") {
      ...
      if ([PLAY_UNLOCK_STATES.WAITING_FOR_AUDIENCE, PLAY_UNLOCK_STATES.WARMING_AUDIENCE, PLAY_UNLOCK_STATES.UNLOCKED].includes(state.sceneZero.unlock.status)) {
        animatePlayUnlockTo100("suitcases-finished");
      }
    }
