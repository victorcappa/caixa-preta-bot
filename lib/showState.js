...
function createStore() {
  ...
  function startSceneZeroParticipantSelection({ invite = "", comments = [], announcement = "" } = {}) {
    ...
    scheduleSceneZeroParticipantAction(() => {
      ...
      state.sceneZero.participantSelection = {
        ...
        rouletteEndsAt: new Date(rouletteStartedAt.getTime() + 7000).toISOString()
      };
      appendSceneZeroAction("participant-roulette-start");
      emitSceneZero("scene-zero-participant-roulette");

      const rouletteComments = state.sceneZero.participantSelection.preparedComments || [];
  ...
  function controlSceneZero(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "set-stage") {
      ...
      if (stage !== state.sceneZero.stage) {
        if (stage !== "participant" && ["preparing", "countdown", "roulette", "selected"].includes(state.sceneZero.participantSelection?.status)) {
          clearSceneZeroParticipantTimers();
          state.sceneZero.participantSelection = {
            ...state.sceneZero.participantSelection,
            status: "cancelled",
      ...
      if (payload.kind === "collection-question" && text) {
        const question = { id: randomUUID(), text, requestedAction: detail || null, timestamp: nowIso };
        state.sceneZero.collection.questions = [...state.sceneZero.collection.questions, question].slice(-30);
        state.sceneZero.collection.lastQuestion = text;
        state.sceneZero.collection.lastRequestedAction = detail || null;
      }
...
export const showState = globalThis[globalKey]?.startGame && globalThis[globalKey]?.controlStructuredGame && globalThis[globalKey]?.cancelPerformanceEvents && globalThis[globalKey]?.startSuitcases && globalThis[globalKey]?.updateInstagram && globalThis[globalKey]?.drawBaralhoMorbidoCard && globalThis[globalKey]?.controlQuedaAviao && globalThis[globalKey]?.controlGlitch && globalThis[globalKey]?.controlDisplayBlackout && globalThis[globalKey]?.controlForcaGShaders && globalThis[globalKey]?.controlSceneZero && globalThis[globalKey]?.prepareSceneZeroParticipantSelection && globalThis[globalKey]?.startSceneZeroParticipantSelection && globalThis[globalKey]?.triggerSceneCue && globalThis[globalKey]?.stopSceneCue && globalThis[globalKey]?.registerProjectionWindow
