...
import { PLAY_UNLOCK_CONFIG, PLAY_UNLOCK_STATES, playUnlockSequenceLines } from "@/data/scene-zero-unlock";
...
const STORE_VERSION = "scene-zero-manual-suitcase-cue-v18";
...
function createStore() {
  ...
  function executeAudienceWarmupAdvance() {
    ...
    if (pending.kind === "sound-check-listen") {
      ...
      const firstWaitingComment = PLAY_UNLOCK_CONFIG.soundCheck.waitingComments?.[0];
      ...
      const comment = PLAY_UNLOCK_CONFIG.soundCheck.waitingComments?.[commentIndex];
      ...
      const nextComment = PLAY_UNLOCK_CONFIG.soundCheck.waitingComments?.[commentIndex + 1];
      if (nextComment) {
        armAudienceWarmupAdvance(
          "sound-check-comment",
  ...
  function drawAudienceWarmupSelection(current, { selectedAction = null, intensity = null } = {}, now = new Date().toISOString()) {
    ...
    state.audienceWarmup = {
      ...
      actionTimer: null,
      recentPromptIds: [...(current.recentPromptIds || []), prompt.id].slice(-8),
      updatedAt: now
    };
    return prompt;
  }
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "resume-questions") {
      ...
      clearAudienceWarmupTimer();
      state.audienceWarmup = { ...current, selectedAction: prompt.action, intensity: prompt.intensity, preview: prompt.text, previewPromptId: prompt.id, recentPromptIds: [...(current.recentPromptIds || []), prompt.id].slice(-8), updatedAt: now };
      publishSelectedAudienceWarmupPrompt(prompt);
      ...
      state.audienceWarmup = {
        ...
        recentPromptIds: current.recentPromptIds?.at(-1) === prompt.id
          ...
          : [...(current.recentPromptIds || []), prompt.id].slice(-8),
        updatedAt: now
      };
      publishAudienceWarmupStep(0);
      message = wasScored
