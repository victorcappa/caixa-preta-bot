...
function createStore() {
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    const requiresBoot = ["generate", "surprise", "send", "trigger-prompt", "next", "repeat"].includes(action);
    if (requiresBoot && [
    ...
    if (action === "select-action") {
      ...
      const prompt = chooseAudienceWarmupPrompt({ action: current.selectedAction, intensity: current.intensity, excludeId: current.previewPromptId });
      clearAudienceWarmupTimer();
      const sequence = createAudienceWarmupSequence({
        text: prompt.text,
      ...
      state.audienceWarmup = { ...current, active: true, preview: prompt.text, previewPromptId: prompt.id, sequence, currentStep: -1, awaitingOperator: !current.automatic, updatedAt: now };
      publishAudienceWarmupStep(0);
