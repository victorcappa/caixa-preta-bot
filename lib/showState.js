...
const STORE_VERSION = "scene-zero-suitcase-blackout-v3";
...
function createStore() {
  ...
  function publishAudienceWarmupStep(stepIndex, { repeated = false } = {}) {
    ...
    const action = getAudienceWarmupAction(sequence.action);
    ...
    const countdownMustContinue = Boolean(sequence.requiresCountdown && nextStep?.kind === "count");
    ...
    if (shouldAutoAdvance) {
      const countdownInstructionDelay = sequence.requiresCountdown && stepIndex === 0
        ? step.text.length * PUBLIC_TYPE_INTERVAL_MS + 700
        : 0;
      const delayMs = Math.max(state.audienceWarmup.intervalMs, countdownInstructionDelay);
  ...
  function generateAudienceWarmupSelection(current, { selectedAction, intensity }, now) {
    ...
    const safeIntensity = ["light", "medium", "strange"].includes(intensity) ? intensity : "light";
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "select-action") {
      ...
      const intensity = ["light", "medium", "strange"].includes(payload.intensity) ? payload.intensity : "light";
