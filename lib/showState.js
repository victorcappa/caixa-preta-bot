...
import {
  createInitialSceneAudioEffectsState,
  normalizeSceneAudioEffects,
  publicSceneAudioEffectsSnapshot
...
function createStore() {
  ...
  const state = {
    ...
    sceneAudioEffects: createInitialSceneAudioEffectsState(),
  ...
  function buildSnapshot({ includePrivate = false } = {}) {
    ...
    return {
      ...
      sceneAudioEffects: publicSceneAudioEffectsSnapshot(state.sceneAudioEffects),
  ...
  function controlSceneAudioEffects({ controllerId, settings = {}, source = "controller" } = {}) {
    const normalizedControllerId = `${controllerId || ""}`.trim();

    if (!normalizedControllerId) {
      return { applied: false, error: "SCENE AUDIO EFFECTS CONTROLLER INVALID" };
    }

    const current = state.sceneAudioEffects[normalizedControllerId] || {};
    const next = normalizeSceneAudioEffects({
      ...current,
      ...settings,
      sequence: Number(current.sequence || 0) + 1
    }, current);
    state.sceneAudioEffects = {
      ...state.sceneAudioEffects,
      [normalizedControllerId]: next
    };

    logPerformance({
      type: "SCENE_AUDIO_EFFECTS",
      source,
      public: false,
      controllerId: normalizedControllerId,
    ...
    return {
      applied: true,
      settings: next,
      state: publicSceneAudioEffectsSnapshot(state.sceneAudioEffects)
    };
  }
...
export const showState = globalThis[globalKey]?.startGame && globalThis[globalKey]?.controlStructuredGame && globalThis[globalKey]?.cancelPerformanceEvents && globalThis[globalKey]?.startSuitcases && globalThis[globalKey]?.updateInstagram && globalThis[globalKey]?.drawBaralhoMorbidoCard && globalThis[globalKey]?.controlQuedaAviao && globalThis[globalKey]?.controlGlitch && globalThis[globalKey]?.controlRobotSound && globalThis[globalKey]?.signalStopAll && globalThis[globalKey]?.controlDisplayBlackout && globalThis[globalKey]?.controlForcaGShaders && globalThis[globalKey]?.controlSceneAudioEffects && globalThis[globalKey]?.controlSceneZero && globalThis[globalKey]?.controlResearch && globalThis[globalKey]?.prepareSceneZeroParticipantSelection && globalThis[globalKey]?.startSceneZeroParticipantSelection && globalThis[globalKey]?.completeSceneZeroMessageTyping && globalThis[globalKey]?.triggerSceneCue && globalThis[globalKey]?.stopSceneCue && globalThis[globalKey]?.updateSceneAudioCue && globalThis[globalKey]?.registerProjectionWindow
