...
function createStore() {
  ...
  function startFirstHumanVerificationPrompt() {
    const prompt = AUDIENCE_WARMUP_PROMPTS.find((candidate) => candidate.id === PLAY_UNLOCK_CONFIG.firstPromptId)
      || AUDIENCE_WARMUP_PROMPTS[0];
    if (!prompt) return;
    const current = state.audienceWarmup;
    const sequence = createAudienceWarmupSequence({
      text: prompt.text,
  ...
  function startHumanVerificationIntro() {
    ...
    playUnlockTimer = setTimeout(() => {
      ...
      startFirstHumanVerificationPrompt();
  ...
  function generateAudienceWarmupSelection(current, { selectedAction, intensity }, now) {
    const action = getAudienceWarmupAction(selectedAction).id;
    const safeIntensity = ["everyday", "public", "intimate", "exposure"].includes(intensity) ? intensity : "everyday";
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    const requiresBoot = ["configure-and-generate", "generate", "surprise", "send", "trigger-prompt", "next", "repeat"].includes(action);
    if (requiresBoot && [
    ...
    if (action === "select-action") {
      ...
      const intensity = ["everyday", "public", "intimate", "exposure"].includes(payload.intensity) ? payload.intensity : "everyday";
      ...
      generateAudienceWarmupSelection(current, {
        selectedAction: payload.selectedAction,
        intensity: payload.intensity
      }, now);
      message = "AÇÃO + INTENSIDADE DEFINIDAS · PERGUNTA ENVIADA";
    } else if (action === "generate") {
      generateAudienceWarmupSelection(current, {
        selectedAction: current.selectedAction,
        intensity: current.intensity
      }, now);
      message = "PERGUNTA GERADA E ENVIADA";
    } else if (action === "surprise") {
      const surpriseCount = Number(current.surpriseCount || 0) + 1;
      const targetIntensity = audienceWarmupSurpriseProfile(surpriseCount).intensity;
      const generated = payload.prompt && typeof payload.prompt === "object" ? payload.prompt : null;
      const prompt = generated?.text ? {
        id: generated.id || `live-surprise-${Date.now()}`,
        text: `${generated.text}`.trim().slice(0, 500),
        action: getAudienceWarmupAction(generated.action).id,
        intensity: targetIntensity,
        countdown: generated.countdown,
        progressValue: generated.progressValue,
        repeatableProgress: false,
        feedbackOverride: generated.feedbackOverride || null,
        steps: generated.steps,
        reaction: false
      } : chooseAudienceWarmupSurprisePrompt({ count: surpriseCount, excludeId: current.previewPromptId });
      clearAudienceWarmupTimer();
      const sequence = createAudienceWarmupSequence({
        text: prompt.text,
        action: prompt.action,
      ...
      state.audienceWarmup = { ...current, active: true, surpriseCount, selectedAction: prompt.action, intensity: prompt.intensity, preview: prompt.text, previewPromptId: prompt.id, sequence, currentStep: -1, awaitingOperator: !current.automatic, updatedAt: now };
      publishAudienceWarmupStep(0);
      message = "COMBINAÇÃO SURPRESA GERADA E ENVIADA";
      ...
      const sequence = createAudienceWarmupSequence({
        ...
        repeatableProgress: payload.repeatableProgress,
        feedbackOverride: payload.feedbackOverride
      });
      state.audienceWarmup = { ...current, active: true, preview: text, sequence, currentStep: -1, awaitingOperator: !current.automatic, updatedAt: now };
      publishAudienceWarmupStep(0);
      message = "FRASE ENVIADA E SEQUÊNCIA PREPARADA";
    } else if (action === "trigger-prompt") {
      const prompt = AUDIENCE_WARMUP_PROMPTS.find((candidate) => candidate.id === payload.promptId);
      ...
      clearAudienceWarmupTimer();
      state.audienceWarmup = { ...current, active: false, awaitingOperator: false, updatedAt: now };
      message = "SEQUÊNCIA CANCELADA";
    } else if (action === "clear") {
      clearAudienceWarmupTimer();
      state.publicMessage = null;
      state.audienceWarmup = { ...current, active: false, sequence: null, currentStep: -1, awaitingOperator: false, display: null, updatedAt: now };
