...
import { chooseAudienceWarmupReaction } from "@/data/audience-warmup-reactions";
...
const STORE_VERSION = "scene-zero-first-provocation-v7";
...
function createStore() {
  ...
  function executeAudienceWarmupAdvance() {
    ...
    if (pending.kind === "sound-check-listen") {
      ...
      emitAudienceWarmup();
    } else if (pending.kind === "minigame-begin-round") {
      const begun = beginAudienceWarmupMinigameRound(state.audienceWarmup.minigame);
      if (!begun.applied) return begun;
      state.audienceWarmup = { ...state.audienceWarmup, minigame: begun.state, updatedAt: new Date().toISOString() };
      emitAudienceWarmup();
  ...
  function publishAudienceWarmupReaction(kind) {
    ...
    const pendingAdvance = rawPendingAdvance?.kind === "reaction-resume"
      ...
      : rawPendingAdvance?.kind === "reaction-complete"
        ...
        : rawPendingAdvance;
    clearAudienceWarmupTimer();
    const text = chooseAudienceWarmupReaction(kind, current.lastReactionText);
    ...
    state.audienceWarmup = {
      ...
      reaction: {
        kind,
        text,
        context: current.display?.text || getAudienceWarmupMinigame(current.minigame?.selectedId)?.name || null,
      ...
      display: {
        ...
        text,
        action: "reaction",
        actionLabel: kind === "many" ? "MUITOS REAGIRAM" : "NINGUÉM REAGIU",
        icon: kind === "many" ? "++" : "--",
        kind: "reaction"
      },
      pendingAdvance,
  ...
  function startAudienceWarmupBriefing() {
    state.audienceWarmup = {
      ...state.audienceWarmup,
      phase: "briefing",
      minigame: {
    ...
    if (state.audienceWarmup.manualMode) {
      armAudienceWarmupAdvance(
        "system-sequence",
        { lines: AUDIENCE_WARMUP_MINIGAME_INTRO, completionKind: "minigame-ready-for-draw", stepIndex: 0 },
        0,
        `FALA 1/${AUDIENCE_WARMUP_MINIGAME_INTRO.length}`
      );
    } else {
      publishAudienceWarmupSystemSequence(AUDIENCE_WARMUP_MINIGAME_INTRO, "minigame-ready-for-draw");
    }
  }
  ...
  function startAudienceSoundCheck() {
    clearPlayUnlockTimers();
    if (state.sceneZero.unlock.status !== PLAY_UNLOCK_STATES.BOOT_FAILED) return false;
  ...
  function controlPlayUnlock(action, payload = {}, { source = "operator" } = {}) {
    ...
    } else if (action === "start-warmup") {
      if (current.status !== PLAY_UNLOCK_STATES.BOOT_FAILED) {
        return { ok: false, error: "BIOS NOT WAITING FOR WARMUP", state: { ...current } };
      }
  ...
  function scheduleAudienceWarmupMinigameExpiry() {
    ...
    const kind = minigame.timer.phase === "countdown"
      ? "minigame-begin-round"
  ...
  function beginAudienceWarmupTapaoExchange() {
    const current = state.audienceWarmup.minigame;
    if (current.selectedId !== "tapao" || !["countdown", "running", "paused"].includes(current.status) || current.round === 2) {
  ...
  function finishAudienceWarmupMinigame({ skipped = false } = {}) {
    ...
    const transition = skipped
      ? ["MINIGAME IGNORADO.", "AQUECIMENTO CONCLUÍDO."]
      : ["FIM.", "TESTE MOTOR CONCLUÍDO.", "AQUECIMENTO CONCLUÍDO."];
    if (state.audienceWarmup.manualMode) {
      armAudienceWarmupAdvance(
        "system-sequence",
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "set-manual-mode") {
      ...
      startAudienceWarmupBriefing();
      message = "PERGUNTAS CONCLUÍDAS · MINI GAME EM PREPARAÇÃO";
    } else if (action === "reaction-none" || action === "reaction-many") {
      const reaction = publishAudienceWarmupReaction(action === "reaction-many" ? "many" : "none");
      if (!reaction.applied) return { ok: false, error: reaction.reason, state: publicAudienceWarmupSnapshot(current) };
      ...
      const result = startAudienceWarmupMinigameCountdown(current.minigame);
      ...
      scheduleAudienceWarmupMinigameExpiry();
      message = result.state.round === 2 ? "SEGUNDO TURNO EM CONTAGEM" : "MINIGAME EM CONTAGEM";
      ...
      const prompt = drawAudienceWarmupSelection(current, { intensity: current.intensity }, now);
      publishSelectedAudienceWarmupPrompt(prompt);
      message = `SORTEADO E DISPARADO: ${prompt.id}`;
