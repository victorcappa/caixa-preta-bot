...
import {
  ...
  SCENE_ZERO_KEEP_BORROWED_OBJECTS_INSTRUCTION,
...
const STORE_VERSION = "hangman-theme-draw-v20";
...
function createStore() {
  ...
  function beginSceneZeroParticipantCountdown(sequence, durationMs = 10000) {
    const selection = state.sceneZero.participantSelection;
    if (selection?.sequence !== sequence || selection.status !== "awaiting_invite") return false;

    const countdownEndsAt = new Date(Date.now() + durationMs).toISOString();
    state.sceneZero.participantSelection = {
      ...selection,
      status: "countdown",
      countdownEndsAt,
    ...
    scheduleSceneZeroParticipantAction(() => {
      ...
      scheduleSceneZeroParticipantAction(() => {
        ...
        const postSelection = (active.preparedPostSelection || []).filter(Boolean);
        const postSchedule = sequentialMessageSchedule([active.preparedAnnouncement, ...postSelection].filter(Boolean), {
          typingIntervalMs: robotTypingIntervalMs(state.robotSound)
        });
        const postOffsetStart = active.preparedAnnouncement ? 1 : 0;
        postSelection.forEach((text, index) => {
          scheduleSceneZeroParticipantAction(() => {
            ...
            state.sceneZero.participantSelection = { ...selected, lastComment: text };
          ...
          }, postSchedule.offsets[index + postOffsetStart] || 0);
        });
        ...
        }, Math.max(5000, postSchedule.totalDurationMs + 500));
      }, rouletteDurationMs);
    }, durationMs);
  ...
  function startSceneZeroParticipantSelection({ invite = "", comments = [], announcement = "", postSelection = [], inviteMessageId = null, skipInvite = false } = {}) {
    const selection = state.sceneZero.participantSelection;
    ...
    const fallbackDelayMs = Math.max(
      20000,
      (normalizedInvite.length * robotTypingIntervalMs(state.robotSound)) + 8000
    );
    state.sceneZero.participantSelection = {
      ...selection,
    ...
    if (skipInvite) {
      beginSceneZeroParticipantCountdown(sequence, 0);
    } else {
      scheduleSceneZeroParticipantAction(() => {
  ...
  function scheduleSceneZeroGincanaTimer() {
    ...
    if (delay === 0) {
      ...
      publishSceneZeroKeepBorrowedObjectsInstruction();
  ...
  function scheduleSceneZeroHangmanTimer() {
    ...
    sceneZeroHangmanTimer = setTimeout(() => {
      ...
      if (!timedOut) return;
      state.sceneZero.suitcaseGame.hangman = timedOut;
      publishSceneZeroHangmanResult(timedOut);
      appendSceneZeroAction("hangman_timeout", timedOut.wordId);
      emitSceneZero("scene-zero-hangman-timeout");
    }, delay);
  }

  function publishSceneZeroKeepBorrowedObjectsInstruction() {
    const gincana = ensureSceneZeroSuitcaseGame().gincana;
    if (gincana.lastComment === SCENE_ZERO_KEEP_BORROWED_OBJECTS_INSTRUCTION) return null;
    gincana.lastComment = SCENE_ZERO_KEEP_BORROWED_OBJECTS_INSTRUCTION;
    return addMessage(
      "assistant",
      SCENE_ZERO_KEEP_BORROWED_OBJECTS_INSTRUCTION,
      "scene-zero-keep-objects"
    );
  }
  ...
  function publishSceneZeroHangmanResult(hangman) {
    ...
    return addMessage("assistant", text, "scene-zero-hangman-result");
  }
  ...
  function controlSceneZero(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "set-stage") {
      ...
      if (stage !== state.sceneZero.stage) {
        if (stage !== "participant" && ["waiting_volunteers", "preparing", "awaiting_invite", "countdown", "roulette", "selected"].includes(state.sceneZero.participantSelection?.status)) {
          clearSceneZeroParticipantTimers();
          state.sceneZero.participantSelection = {
            ...state.sceneZero.participantSelection,
            status: "cancelled",
      ...
      const timer = suitcaseGame.gincana.timer;
      if (!suitcaseGame.gincana.currentTask || !["idle", "running", "paused"].includes(timer.status)) {
        return { applied: false, error: "SCENE ZERO GINCANA CANNOT ADD TIME", state: publicSceneZeroSnapshot(state.sceneZero) };
      }
      ...
      const remainingSeconds = Math.min(120, liveRemaining + addedSeconds);
      const durationSeconds = Math.min(120, Math.max(Number(timer.durationSeconds) || 0, 0) + addedSeconds);
      suitcaseGame.gincana.durationSeconds = durationSeconds;
      suitcaseGame.gincana.timer = {
        ...timer,
        durationSeconds,
        remainingSeconds,
        endsAt: timer.status === "running" ? new Date(now.getTime() + remainingSeconds * 1000).toISOString() : timer.endsAt,
        sequence: timer.sequence + 1
      };
      appendSceneZeroAction(action, `+${addedSeconds}s`);
      if (timer.status === "running") scheduleSceneZeroGincanaTimer();
      ...
      suitcaseGame.hangman = guessed;
      if (guessed.status !== "active") publishSceneZeroHangmanResult(guessed);
      appendSceneZeroAction(action, `${payload.guess || ""} / ${guessed.lastResult}`);
      if (guessed.status !== "active") clearSceneZeroHangmanTimer();
      ...
      if (failed.status !== "active") publishSceneZeroHangmanResult(failed);
      appendSceneZeroAction(action, `${failed.errorCount}`);
      if (failed.status !== "active") clearSceneZeroHangmanTimer();
      ...
      suitcaseGame.hangman = revealed;
      publishSceneZeroHangmanResult(revealed);
      ...
      suitcaseGame.hangman = won;
      publishSceneZeroHangmanResult(won);
      ...
      suitcaseGame.hangman = lost;
      publishSceneZeroHangmanResult(lost);
  ...
  function armAudienceWarmupAdvance(kind, payload = {}, delayMs = 0, label = kind) {
    ...
    state.audienceWarmup = {
      ...state.audienceWarmup,
      pendingAdvance,
      awaitingOperator: Boolean(state.audienceWarmup.manualMode),
      updatedAt: new Date().toISOString()
    };
    emitAudienceWarmup();
    if (!state.audienceWarmup.manualMode) {
      audienceWarmupTimer = setTimeout(() => {
        audienceWarmupTimer = null;
        if (state.audienceWarmup.manualMode || state.audienceWarmup.pendingAdvance?.id !== pendingAdvance.id) {
          if (state.audienceWarmup.manualMode) {
  ...
  function executeAudienceWarmupAdvance() {
    ...
    if (pending.kind === "sound-check-listen") {
      ...
      publishAudienceWarmupSystemSequence(pending.payload.lines, pending.payload.completionKind, pending.payload.stepIndex);
  ...
  function publishAudienceWarmupSystemText(text, kind = "system") {
    const message = addMessage("assistant", text, "audience-warmup-minigame");
    const now = new Date().toISOString();
    state.audienceWarmup = {
      ...state.audienceWarmup,
      active: true,
  ...
  function publishAudienceWarmupSystemSequence(lines, completionKind, stepIndex = 0) {
    const text = lines[stepIndex];
    if (!text) {
      if (completionKind) armAudienceWarmupAdvance(completionKind, {}, 0, "CONCLUIR ETAPA");
      return;
    }
    publishAudienceWarmupSystemText(text, "transition");
    const readingMs = Math.max(
      AUDIENCE_WARMUP_MINIGAME_STEP_DURATION_MS,
      text.length * robotTypingIntervalMs(state.robotSound) + 650
    );
    if (stepIndex + 1 < lines.length) {
      armAudienceWarmupAdvance("system-sequence", { lines, completionKind, stepIndex: stepIndex + 1 }, readingMs, `FALA ${stepIndex + 2}/${lines.length}`);
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "resume-questions") {
      ...
      state.audienceWarmup = {
        ...current,
        manualMode,
        awaitingOperator: manualMode && Boolean(current.pendingAdvance),
        updatedAt: now
      };
      if (!manualMode && current.pendingAdvance) {
        armAudienceWarmupAdvance(
      ...
      } else if (current.phase === "questions" && current.sequence?.completionKind === "first-provocation" && current.actionTimer) {
        clearAudienceWarmupTimer();
        completeAudienceWarmupTimedAction();
        const pending = state.audienceWarmup.pendingAdvance;
