...
import { PLAY_UNLOCK_CONFIG, PLAY_UNLOCK_STATES } from "@/data/scene-zero-unlock";
...
function createStore() {
  ...
  function schedulePlayUnlockSequence(source = "progress") {
    ...
    const advanceSequence = () => {
      ...
      if (nextIndex >= PLAY_UNLOCK_CONFIG.unlockLines.length) {
      ...
      playUnlockTimer = setTimeout(advanceSequence, PLAY_UNLOCK_CONFIG.unlockStepMs);
    };
    playUnlockTimer = setTimeout(advanceSequence, PLAY_UNLOCK_CONFIG.unlockStepMs);
  }
  ...
  function controlAudienceWarmup(action, payload = {}, { source = "operator" } = {}) {
    ...
    if (action === "select-action") {
      ...
      const prompt = chooseAudienceWarmupPrompt({ excludeId: current.previewPromptId });
      clearAudienceWarmupTimer();
      const sequence = createAudienceWarmupSequence({
        text: prompt.text,
        action: prompt.action,
      ...
      state.audienceWarmup = { ...current, active: true, selectedAction: prompt.action, intensity: prompt.intensity, preview: prompt.text, previewPromptId: prompt.id, sequence, currentStep: -1, awaitingOperator: !current.automatic, updatedAt: now };
      publishAudienceWarmupStep(0);
