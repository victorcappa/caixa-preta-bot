...
function normalizeInterpretedCommand(interpreted = {}, fallback) {
  ...
  const actionsWithoutUsername = new Set(["analyze_current", "watch_reels", "send_direct_latest", "send_direct_thread"]);
...
function extractDirectMessage(content) {
  if (!/\b(direct|dm|mensagem|message|chat|grupo|group|conversa)\b/i.test(content)) {
    return "";
  }
