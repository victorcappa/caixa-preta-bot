...
export function parseInstagramCommand(content = "") {
  ...
  if (/^(?:abrir|open|login|entrar)$/i.test(trimmed)) {
    return {
      valid: true,
      action: "open",
      username: "",
...
export async function interpretInstagramCommand(content = "") {
  ...
  if (fallback.valid && fallback.action === "open") {
    return fallback;
  }
...
function normalizeInterpretedCommand(interpreted = {}, fallback) {
  ...
  const actionsWithoutUsername = new Set(["open", "analyze_current", "watch_reels", "open_directs", "send_direct_latest", "send_direct_thread"]);
