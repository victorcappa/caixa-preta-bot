...
export class InstagramController {
  ...
  async openLatestProfileMedia(username) {
    ...
    await this.page.waitForURL(/instagram\.com\/(p|reel|tv)\//i, { timeout: 12000 }).catch(() => {});
    await Promise.race([
      this.page.getByRole("dialog").first().waitFor({ state: "visible", timeout: 8000 }),
      this.page.locator("article").first().waitFor({ state: "visible", timeout: 8000 }),
      this.page.getByRole("main").waitFor({ state: "visible", timeout: 8000 })
    ]).catch(() => {});
    this.currentUrl = this.page.url();
    await this.dismissKnownModals();
    return /instagram\.com\/(p|reel|tv)\//i.test(this.currentUrl);
  }
  ...
  async getFollowButtonState() {
    ...
    for (const candidate of candidates) {
      ...
      const locators = [
        ...scopedContexts.map((context) => context.getByRole("button", { name }).first()),
