...
const FRAME_CAPTURE_TIMEOUT_MS = 3000;
...
export function getInstagramConfig(env = process.env) {
  const embedded = parseBoolean(env.INSTAGRAM_EMBEDDED, true);
  ...
  return {
    ...
    embedded,
    headless: parseBoolean(env.INSTAGRAM_HEADLESS, embedded),
    ...
    streamFps: parseInteger(env.INSTAGRAM_STREAM_FPS, 30, 4, 30),
    streamQuality: parseInteger(env.INSTAGRAM_STREAM_QUALITY, 55, 35, 90),
    allowedProfiles,
...
export class InstagramController {
  constructor({ config = getInstagramConfig(), reporter = null } = {}) {
    ...
    this.pageConfigured = false;
    this.lastFrame = null;
    this.abortOperation = null;
  }
  ...
  async init() {
    ...
    this.context = await chromium.launchPersistentContext(this.config.profileDir, {
      ...
      args: [
        ...
        "--disable-infobars",
        ...(this.config.embedded && !this.config.headless ? [
  ...
  async open() {
    ...
    await this.gotoPage(INSTAGRAM_HOME, "abrir instagram.com");
    ...
    await this.captureJpegFrame({ allowCommandInProgress: true }).catch(() => {});
  ...
  async openProfile(username) {
    ...
    await this.gotoPage(`https://www.instagram.com/${safeUsername}/`, `abrir @${safeUsername}`);
    await this.applyAudioMuted().catch(() => {});
    this.currentUrl = this.page.url();
  ...
  async openReels() {
    ...
    try {
      ...
      await this.theatricalDelay();
      await this.gotoPage("https://www.instagram.com/reels/", "abrir reels");
      await this.applyAudioMuted().catch(() => {});
      this.currentUrl = this.page.url();
      await this.captureJpegFrame({ allowCommandInProgress: true }).catch(() => {});
      await this.dismissKnownModals();
  ...
  async applyAudioMuted() {
    ...
    await this.page.evaluate((nextMuted) => {
      ...
      window.__caixaPretaApplyInstagramAudio = () => {
      ...
      if (!window.__caixaPretaInstagramAudioObserver) {
        ...
        window.__caixaPretaInstagramAudioObserver.observe(document.documentElement, {
          ...
          subtree: true,
          attributes: true,
          attributeFilter: ["muted", "src", "class", "aria-label"]
        });
      }

      if (!window.__caixaPretaInstagramAudioInterval) {
        window.__caixaPretaInstagramAudioInterval = window.setInterval(() => {
          window.__caixaPretaApplyInstagramAudio?.();
        }, 500);
      }
  ...
  async stopAllRoutines() {
    this.stopReelsAutoplay({ silent: true });
    this.abortOperation?.(new Error("INSTAGRAM_STOPPED"));
    this.abortOperation = null;
    this.actionInProgress = false;
    this.commandInProgress = false;
  ...
  async captureJpegFrame({ quality = this.config.streamQuality, allowCommandInProgress = false } = {}) {
    if (this.actionInProgress || (this.commandInProgress && !allowCommandInProgress)) {
      throw new Error(this.actionInProgress ? "INSTAGRAM_ACTION_IN_PROGRESS" : "INSTAGRAM_COMMAND_IN_PROGRESS");
    }
  ...
  getCachedFrame({ maxAgeMs = 5000 } = {}) {
    if (!this.lastFrame || Date.now() - this.lastFrame.capturedAt > maxAgeMs) {
      return null;
    }

    return {
      image: this.lastFrame.image,
      viewport: this.lastFrame.viewport
    };
  }
  ...
  async gotoPage(url, label = "navegar") {
    let abortOperation = null;
    const stopped = new Promise((_, reject) => {
      abortOperation = reject;
    });
    ...
    try {
      await Promise.race([navigation, stopped]);
    } catch (error) {
      this.currentUrl = this.page?.url?.() || this.currentUrl;
      this.lastError = publicError(error);
      if (error.message === "INSTAGRAM_STOPPED") {
        this.updateStatus(this.status, "comando Instagram interrompido");
      } else {
        this.updateStatus(INSTAGRAM_STATUS.ERROR, `${label} falhou: ${this.lastError}`);
      }
      throw error;
    } finally {
      if (this.abortOperation === abortOperation) {
        this.abortOperation = null;
      }
    }
  }
  ...
  async waitForEmbeddedFrameReady(reason = "aguardando iframe") {
    ...
    while (Date.now() < deadline) {
      try {
        await this.captureJpegFrame({
          quality: Math.min(this.config.streamQuality, 55),
          allowCommandInProgress: true
        });
