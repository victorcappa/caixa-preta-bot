...
export class InstagramController {
  ...
  async openReels() {
    ...
    try {
      ...
      this.updateStatus(INSTAGRAM_STATUS.READY, "reels aberto no iframe");
      return { status: "ready", message: "INSTAGRAM: reels aberto no iframe" };
    } finally {
      this.commandInProgress = false;
    }
  }
  ...
  async commentLatestMedia(username, comment) {
    ...
    this.commandInProgress = true;
    this.lastAction = `comment latest @${safeUsername}`;
    this.targetProfile = safeUsername;
    ...
    try {
      ...
      if (!await this.openLatestProfileMedia(safeUsername)) {
        ...
        return { status: "unconfirmed", message: "INSTAGRAM: nao foi possivel comentar na ultima midia" };
      }
