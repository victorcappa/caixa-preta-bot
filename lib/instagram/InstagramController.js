...
const SAFE_LOGIN_CONTINUE_PATTERNS = [
  /^Continue$/i,
  /^Continuar$/i,
  /^Continue as\b/i,
  /^Continuar como\b/i
...
export function parseGoogleGuidance(guidance = "") {
  ...
  const durationSeconds = durationMatch
    ...
    : 0;
  const searchMatch = normalized.match(/\b(?:buscar|busque|pesquisar|pesquise|procurar|procure)(?:\s+(?:no\s+google|na\s+internet|por|sobre))?\s+(.+?)(?=\s+e\s+(?:escolher|abrir|acessar|entrar|ler|visitar|clicar|comentar|comente)\b|$)/iu);
  const query = `${searchMatch?.[1] || normalized}`
    .replace(/\s+e\s+(?:comentar|comente)(?:\s+.+)?$/iu, "")
    .replace(/[.,;:]\s*$/u, "")
    .trim()
    .slice(0, 300);
  const preferNews = /\b(?:not[ií]cia|notici[aá]rio|jornal|reportagem|mat[eé]ria)\b/iu.test(normalized);
  const wantsComment = /\b(?:comentar|comente|comenta|coment[aá]rio|sarcasmo|sarc[aá]stic[oa])\b/iu.test(normalized);
  const openResult = wantsComment || /\b(?:escolher|abrir|acessar|entrar|ler|visitar|clicar)\b/iu.test(normalized);
...
export function selectGuidedGoogleResults(rawResults = [], { preferNews = false, limit = 2 } = {}) {
  const candidates = rawResults
    ...
    .filter(({ url }) => {
      ...
      const referenceHost = /(^|\.)(wikipedia\.org|wikimedia\.org|britannica\.com|dicio\.com\.br|significados\.com\.br)$/i.test(hostname);
      return !archivePath && !(preferNews && referenceHost) && !/(^|\.)(google\.|bing\.com|microsoft\.com|msn\.com|instagram\.com|facebook\.com|tiktok\.com|x\.com|twitter\.com|linkedin\.com)/i.test(hostname);
    });
...
export class InstagramController {
  constructor({ config = getInstagramConfig(), reporter = null, credentialsLoader = loadInstagramCredentials } = {}) {
    ...
    this.embeddedInputInProgress = 0;
    this.embeddedPanelSequence = 0;
    this.commandInProgress = false;
    this.reelsAutoplayTimer = null;
    this.reelsAutoplayGeneration = 0;
    this.reelsAutoplayActive = false;
    this.reelsOpening = false;
    this.reelsPerformance = null;
    this.reelsFeedIndex = 0;
    this.lastReelComment = null;
    this.audioMuted = true;
    this.browserMode = "instagram";
    this.research = null;
    this.googleGuidanceGeneration = 0;
    this.googleChallengeDetected = false;
    this.secondaryPage = null;
    this.secondaryBrowserLabel = null;
  }
  ...
  getStatus() {
    return {
      ...
      embeddedPanelSequence: this.embeddedPanelSequence,
      ...
      sessionAuthenticated: this.sessionAuthenticated,
      ...
      reelsPerformance: this.reelsPerformance ? {
        posted: this.reelsPerformance.posted,
        spoken: this.reelsPerformance.spoken
      } : null,
      lastReelComment: this.lastReelComment,
      audioMuted: this.audioMuted,
      browserMode: this.browserMode,
      research: this.research ? { ...this.research } : null,
      secondaryBrowser: this.secondaryPage ? {
        active: true,
        label: this.secondaryBrowserLabel || "SECUNDÁRIA",
  ...
  async ensureInstagramSession({ automatic = true } = {}) {
    await this.init();
    this.ensurePage();
    await this.configurePage();

    if (!/^https:\/\/(?:www\.)?instagram\.com\//i.test(this.page.url())) {
      this.updateStatus(INSTAGRAM_STATUS.NAVIGATING, "INSTAGRAM: preparando sessao autenticada");
      await this.page.goto(INSTAGRAM_HOME, { waitUntil: "domcontentloaded" });
      this.currentUrl = this.page.url();
    }
    ...
    if (!automatic) {
      this.requestEmbeddedPanel();
      this.updateStatus(INSTAGRAM_STATUS.LOGIN_REQUIRED, "LOGIN MANUAL: toque em Continue/Continuar no painel antes de abrir o perfil");
      return { status: "login_required", message: "INSTAGRAM: conclua o login manual antes de abrir o perfil" };
    }

    return this.attemptAutomaticLogin();
  }

  async prepareManualLogin() {
    this.requestEmbeddedPanel();
    this.stopReelsAutoplay({ silent: true });
    await this.closeSecondaryPage();
    await this.init();
    this.ensurePage();
    await this.configurePage();
    this.browserMode = "instagram_manual_login";
    this.research = null;
    this.lastAction = "manualLogin";
    this.targetProfile = null;
    this.updateStatus(INSTAGRAM_STATUS.NAVIGATING, "LOGIN MANUAL: abrindo Instagram sem preencher credenciais");
    await this.page.goto(INSTAGRAM_HOME, { waitUntil: "domcontentloaded" });
    this.currentUrl = this.page.url();
    await this.dismissKnownModals();
    ...
    const session = await this.getSessionState();
    if (session === "authenticated") {
      this.updateStatus(INSTAGRAM_STATUS.READY, "LOGIN MANUAL: sessao autenticada; perfis liberados");
      return { status: "ready", message: "INSTAGRAM: sessão autenticada; perfis liberados" };
    }

    if (session === "manual_intervention") {
      this.updateStatus(INSTAGRAM_STATUS.MANUAL_INTERVENTION, "LOGIN MANUAL: conclua a verificacao no painel");
      return { status: "manual_intervention", message: "INSTAGRAM: conclua a verificação manual no painel" };
    }

    this.updateStatus(INSTAGRAM_STATUS.LOGIN_REQUIRED, "LOGIN MANUAL: toque em Continue/Continuar e conclua o login no painel");
    return { status: "login_required", message: "INSTAGRAM: toque em Continue/Continuar e conclua o login no painel" };
  }
  ...
  async searchPublicWeb(query, { preferGoogle = true, news = false } = {}) {
    if (preferGoogle && !this.googleChallengeDetected) {
      const googleNews = news ? "&tbm=nws&source=lnms" : "";
      await this.page.goto(`https://www.google.com/search?q=${encodeURIComponent(query)}&hl=pt-BR${googleNews}`, {
        waitUntil: "domcontentloaded",
        timeout: 20000
      });
      if (!await this.isGoogleChallengePage(this.page)) {
        await this.page.waitForTimeout(850);
        if (!await this.isGoogleChallengePage(this.page)) return "google";
      }
      this.googleChallengeDetected = true;
      this.updateStatus(INSTAGRAM_STATUS.NAVIGATING, `GOOGLE PEDIU VERIFICAÇÃO; USANDO BING${news ? " NEWS" : ""}`);
    } else if (preferGoogle && this.googleChallengeDetected) {
      this.updateStatus(INSTAGRAM_STATUS.NAVIGATING, `GOOGLE JÁ BLOQUEOU ESTA SESSÃO; USANDO BING${news ? " NEWS" : ""}`);
    }
  ...
  async researchPerson(name = "", { knownInstagramHandle = "" } = {}) {
    ...
    try {
      ...
      if (links.instagram) {
        ...
        this.research.step = "instagram_login";
        const login = await this.ensureInstagramSession({ automatic: false });
        if (login.status !== "ready") {
          this.research.step = login.status;
          return { ...login, person, links };
        }
  ...
  startGoogleGuidance(guidance = "", { onComplete = null } = {}) {
    const plan = typeof guidance === "string" ? parseGoogleGuidance(guidance) : guidance;
    ...
    void this.followGoogleGuidance(plan, generation).then(async (result) => {
      if (result.status === "ready" && typeof onComplete === "function") {
        this.updateStatus(INSTAGRAM_STATUS.ACTING, "GOOGLE: PROCESSANDO INFORMAÇÕES LIDAS");
        await onComplete(result);
        if (generation === this.googleGuidanceGeneration) {
          this.updateStatus(
            INSTAGRAM_STATUS.READY,
            result.articles?.length ? "GOOGLE: COMENTÁRIO CONCLUÍDO" : "GOOGLE: PROCESSAMENTO CONCLUÍDO"
          );
        }
      }
    }).catch((error) => {
  ...
  async openPublicInstagramAlongside(subject = "") {
    ...
    this.secondaryBrowserLabel = "INSTAGRAM";
  ...
  async openGoogleAlongside(guidance = "") {
    const plan = typeof guidance === "string" ? parseGoogleGuidance(guidance) : guidance;
    if (!plan.guidance) return { status: "invalid", message: "GOOGLE: escreva uma orientação" };
    const openBlankGoogle = /\bgoogle\b/iu.test(plan.guidance) && !/\b(?:buscar|busque|pesquisar|pesquise|procurar|procure)\b/iu.test(plan.guidance);
    ...
    const page = await this.context.newPage();
    this.secondaryPage = page;
    this.secondaryBrowserLabel = "GOOGLE 2";
    await page.setViewportSize(this.config.viewport).catch(() => {});
    await page.setExtraHTTPHeaders({ "User-Agent": MOBILE_USER_AGENT, "Sec-CH-UA-Mobile": "?1" }).catch(() => {});
    this.browserMode = "browser_command";
    this.research = {
      ...(this.research || {}),
      guidance: plan.guidance,
      secondaryQuery: openBlankGoogle ? "" : plan.query,
      step: "secondary_google_search",
      startedAt: nowIso()
    };
    this.updateStatus(INSTAGRAM_STATUS.NAVIGATING, openBlankGoogle ? "GOOGLE 2: ABRINDO NOVA ABA" : `GOOGLE 2: BUSCANDO ${plan.query}`);
    const googleNews = plan.preferNews ? "&tbm=nws&source=lnms" : "";
    const googleUrl = openBlankGoogle
      ? "https://www.google.com/?hl=pt-BR"
      : `https://www.google.com/search?q=${encodeURIComponent(plan.query)}&hl=pt-BR${googleNews}`;
    await page.goto(googleUrl, {
      waitUntil: "domcontentloaded",
      timeout: 20000
    });
    await page.waitForTimeout(500);
    const googleBlocked = await this.isGoogleChallengePage(page);
    if (googleBlocked) {
      this.googleChallengeDetected = true;
      const bingUrl = openBlankGoogle
        ? "https://www.bing.com/?setlang=pt-br"
        : `https://www.bing.com${plan.preferNews ? "/news/search" : "/search"}?q=${encodeURIComponent(plan.query)}&setlang=pt-br`;
      this.updateStatus(INSTAGRAM_STATUS.NAVIGATING, `GOOGLE PEDIU VERIFICAÇÃO; USANDO BING${plan.preferNews ? " NEWS" : ""}`);
    ...
    this.research = { ...this.research, step: "secondary_google_ready" };
    this.updateStatus(INSTAGRAM_STATUS.READY, "GOOGLE 2: RESULTADOS ABERTOS EM NOVA ABA");
    return { status: "ready", message: "GOOGLE: nova aba aberta", plan, url: page.url() };
  }
  ...
  async attemptAutomaticLogin() {
    ...
    try {
      let usedRememberedAccount = false;

      if (!/\/accounts\/login/i.test(this.page.url())) {
        usedRememberedAccount = await this.pressSafeLoginContinue();
        if (usedRememberedAccount) {
          await this.page.waitForTimeout(700);
          this.currentUrl = this.page.url();

          const continuedSession = await this.getSessionState();
          if (continuedSession === "authenticated") {
            await this.dismissKnownModals();
          ...
          if (continuedSession === "manual_intervention") {
      ...
      let usernameInput = this.page.locator("input[name='username']").first();
      let passwordInput = this.page.locator("input[name='password']").first();
      const loginFormExpected = usedRememberedAccount || /\/accounts\/login/i.test(this.page.url());
      let passwordReady = await waitForVisible(passwordInput, loginFormExpected ? 10000 : 800);

      if (!passwordReady && !/\/accounts\/login/i.test(this.page.url())) {
        await this.page.goto(INSTAGRAM_LOGIN, { waitUntil: "domcontentloaded" });
        this.currentUrl = this.page.url();
        usernameInput = this.page.locator("input[name='username']").first();
        passwordInput = this.page.locator("input[name='password']").first();
        passwordReady = await waitForVisible(passwordInput, 10000);
      }
      ...
      if (!passwordReady) {
        this.updateStatus(INSTAGRAM_STATUS.MANUAL_INTERVENTION, "campo de senha nao reconhecido");
        return { status: "manual_intervention", message: "INSTAGRAM: intervencao manual necessaria" };
      }
      ...
      const namedSubmit = this.page.getByRole("button", { name: /^Log in$|^Entrar$|^Continue$|^Continuar$/i }).first();
  ...
  async waitForLoginResult({ timeoutMs = 25000 } = {}) {
    ...
    while (Date.now() < deadline) {
      const session = await this.getSessionState();
      if (session === "manual_intervention") return session;

      if (continueClicks < 2 && await this.pressSafeLoginContinue()) {
        continueClicks += 1;
        await this.page.waitForTimeout(700);
        continue;
      }
  ...
  async pressSafeLoginContinue() {
    if (await this.hasManualInterventionSignal()) return false;

    for (const pattern of SAFE_LOGIN_CONTINUE_PATTERNS) {
      const button = this.page.getByRole("button", { name: pattern }).first();
      if (!await button.isVisible().catch(() => false)) continue;

      this.updateStatus(INSTAGRAM_STATUS.ACTING, "login: pressionando Continuar");
  ...
  async openReels({ manualLogin = false } = {}) {
    if (this.commandInProgress || this.actionInProgress) {
      return { status: "busy", message: "INSTAGRAM: acao em andamento" };
    }

    this.commandInProgress = true;
    this.reelsOpening = true;
    const openingGeneration = this.reelsAutoplayGeneration;
    this.lastAction = "watchReels";
    this.targetProfile = null;
    this.updateStatus(INSTAGRAM_STATUS.STARTING, "reels solicitados");

    try {
      const opened = manualLogin
        ? await this.ensureInstagramSession({ automatic: false })
        : await this.open();
      if (openingGeneration !== this.reelsAutoplayGeneration) {
        await this.close();
        return { status: "stopped", message: "INSTAGRAM: reels interrompidos" };
      }
  ...
  startReelsPerformance(onReel) {
    if (!this.reelsAutoplayActive || typeof onReel !== "function") return false;
    this.reelsPerformance = {
      onReel,
      posted: 0,
      spoken: 0,
      seen: new Set(),
    ...
    this.updateStatus(INSTAGRAM_STATUS.READY, "reels da peça: comentários ativos");
    if (this.reelsAutoplayTimer) clearTimeout(this.reelsAutoplayTimer);
    this.reelsAutoplayTimer = null;
    this.scheduleNextReelAutoplayStep(500, this.reelsAutoplayGeneration);
    return true;
  }
  ...
  async advanceReelsAutoplay(generation) {
    ...
    const performance = this.reelsPerformance;
    if (performance && Date.now() - performance.lastChatAt >= 12000) {
      const reelTarget = await this.getCurrentReelTarget();
      const reelUrl = reelTarget?.url || null;
      const reelKey = reelTarget?.key || null;
      if (!reelKey) this.lastReelComment = {
        status: "unavailable",
        reason: "Reel ou botão de comentário não identificado",
        at: nowIso()
      };
      else if (this.lastReelComment?.status === "unavailable") this.lastReelComment = null;
      const canPost = Boolean(reelKey && !performance.seen.has(reelKey)
        && Date.now() - performance.lastPostAt >= 20000);
      if (canPost) performance.seen.add(reelKey);
      const isActive = () => this.reelsAutoplayActive
        && generation === this.reelsAutoplayGeneration
        && this.reelsPerformance === performance;
      try {
        const result = await performance.onReel({ reelUrl, reelKey, canPost, isActive });
        if (!isActive()) return;
        performance.lastChatAt = Date.now();
        performance.spoken += 1;
        if (canPost) {
          performance.lastPostAt = Date.now();
          if (result?.status === "commented") performance.posted += 1;
          this.lastReelComment = {
        ...
        this.lastError = publicError(error);
        performance.lastChatAt = Date.now();
        if (canPost) performance.lastPostAt = Date.now();
        if (canPost) this.lastReelComment = { status: "error", reason: this.lastError, at: nowIso() };
        this.updateStatus(INSTAGRAM_STATUS.ERROR, "comentário desta rodada falhou; tentando no próximo Reel");
      }
    }

    if (!this.reelsAutoplayActive || generation !== this.reelsAutoplayGeneration) return;
    await this.performEmbeddedSwipe("up");
    this.reelsFeedIndex += 1;
    await this.applyAudioMuted().catch(() => {});
    const nextDelayMs = await this.getCurrentReelWatchDelayMs();
    const spoken = this.reelsPerformance?.spoken;
    const posted = this.reelsPerformance?.posted;
    this.updateStatus(this.lastReelComment && !["commented", "stopped"].includes(this.lastReelComment.status)
      ? INSTAGRAM_STATUS.MANUAL_INTERVENTION : INSTAGRAM_STATUS.READY, spoken == null
      ? `assistindo reels; proximo em ${Math.round(nextDelayMs / 1000)}s`
      : `assistindo reels · ${spoken} fala${spoken === 1 ? "" : "s"} no chat · ${posted} comentário${posted === 1 ? "" : "s"} enviado${posted === 1 ? "" : "s"}${this.lastReelComment && !["commented", "stopped"].includes(this.lastReelComment.status) ? ` · comentário: ${this.lastReelComment.reason || this.lastReelComment.status}` : ""}`);
    this.scheduleNextReelAutoplayStep(nextDelayMs, generation);
  }

  async getCurrentReelTarget() {
    ...
    const candidate = await this.page.evaluate(() => {
      ...
      const visible = videos
        .map((video) => {
          ...
          return { video, area: Math.max(0, rect.width) * overlap };
        })
        .sort((a, b) => b.area - a.area)[0];
      if (!visible || visible.area < 1000) {
        const commentIcon = [...document.querySelectorAll("svg[aria-label]")].some((icon) => {
          const rect = icon.getBoundingClientRect();
          return /comment|coment.rio|comentar/i.test(icon.getAttribute("aria-label") || "")
            && rect.width > 0 && rect.height > 0 && rect.bottom > 0 && rect.top < innerHeight;
        });
        return commentIcon ? { url: null, key: null } : null;
      }
      const videoRect = visible.video.getBoundingClientRect();
      let node = visible.video;
      for (let depth = 0; node && depth < 7; depth += 1, node = node.parentElement) {
        const anchors = node.matches?.("a[href*='/reel/']")
          ? [node]
          : [...(node.querySelectorAll?.("a[href*='/reel/']") || [])];
        for (const anchor of anchors) {
          const url = new URL(anchor.href, location.href);
          const rect = anchor.getBoundingClientRect();
          const overlapsVideo = rect.right > videoRect.left && rect.left < videoRect.right
            && rect.bottom > videoRect.top && rect.top < videoRect.bottom;
          if (overlapsVideo && reelPath.test(url.pathname)) return { url: url.href, key: url.pathname };
        }
      }
      if (reelPath.test(location.pathname)) return { url: location.href, key: location.pathname };
      const video = visible.video;
      const source = video.currentSrc || video.src || video.querySelector("source")?.src || video.poster;
      return { url: null, key: source ? `video:${source}` : null };
    }).catch(() => null);
    if (!candidate) return null;
    if (!candidate.url) return { url: null, key: candidate.key || `feed:${this.reelsAutoplayGeneration}:${this.reelsFeedIndex}` };
    try {
      const url = new URL(candidate.url);
      if (!["www.instagram.com", "instagram.com"].includes(url.hostname)
        || !/^\/reel\/[A-Za-z0-9_-]+\/?$/.test(url.pathname)) return null;
      return { url: `https://www.instagram.com${url.pathname}`, key: url.pathname };
    } catch {
      return null;
    }
  }

  async getCurrentReelUrl() {
    return (await this.getCurrentReelTarget())?.url || null;
  }

  async commentCurrentReel(reelUrl, comment, { isActive = () => true, reelKey = null } = {}) {
    const safeComment = normalizeCommentText(comment);
    const url = reelUrl ? new URL(reelUrl) : null;
    if (!safeComment || (url && (!["instagram.com", "www.instagram.com"].includes(url.hostname)
      || !/^\/reel\/[A-Za-z0-9_-]+\/?$/.test(url.pathname)))) {
      throw new Error("INSTAGRAM_REEL_COMMENT_INVALID");
    }
    if (!isActive() || !this.page) return { status: "stopped" };
    if (!this.sessionAuthenticated) return { status: "unconfirmed", reason: "sessão do Instagram sem login" };

    this.actionInProgress = true;
    const feedUrl = this.page.url();
    try {
      this.lastAction = "comment current reel";
      this.updateStatus(INSTAGRAM_STATUS.NAVIGATING, "abrindo comentários do Reel");
      if (url) {
        await this.page.goto(`https://www.instagram.com${url.pathname}`, { waitUntil: "domcontentloaded" });
        this.currentUrl = this.page.url();
      } else if (reelKey && (await this.getCurrentReelTarget())?.key !== reelKey) {
        return { status: "stopped" };
      }
      if (!isActive()) return { status: "stopped" };
      if (!this.sessionAuthenticated) return { status: "unconfirmed", reason: "sessão do Instagram sem login" };
      if (url) await this.dismissKnownModals();
      const openedComments = await this.openCommentsSurface();
      const input = await this.findCommentInput();
      if (!input || !isActive()) return { status: "unconfirmed", reason: openedComments ? "campo de comentário ausente" : "comentários não abriram" };
      this.updateStatus(INSTAGRAM_STATUS.ACTING, "publicando comentário no Reel");
      await this.typeCommentText(input, safeComment);
      if (!isActive()) return { status: "stopped" };
      await this.submitComment(input);
      const posted = await this.page.getByText(safeComment, { exact: true }).last()
        .waitFor({ state: "visible", timeout: 5000 }).then(() => true).catch(() => false);
      return { status: posted ? "commented" : "unconfirmed", ...(posted ? {} : { reason: "envio não confirmado no Reel" }) };
    } finally {
      if (isActive() && feedUrl !== this.page.url()) {
  ...
  requestEmbeddedPanel() {
    this.embeddedPanelSequence += 1;
    return this.embeddedPanelSequence;
  }
  ...
  async captureFrame({ fast = false, pane = "primary" } = {}) {
    if (this.embeddedInputInProgress > 0) {
      throw new Error("INSTAGRAM_INPUT_IN_PROGRESS");
    }
  ...
  async captureJpegFrame({ quality = this.config.streamQuality, skipPageSetup = false } = {}) {
    if (this.actionInProgress || this.embeddedInputInProgress > 0) {
  ...
  async sendEmbeddedInput(input = {}) {
    this.embeddedInputInProgress += 1;
    try {
      return await this.performEmbeddedInput(input);
    } finally {
      this.embeddedInputInProgress = Math.max(0, this.embeddedInputInProgress - 1);
    }
  }

  async performEmbeddedInput(input = {}) {
  ...
  async configurePage() {
    ...
    const desiredViewport = this.config.viewport;
    ...
    if (
      ...
      this.updateStatus(this.status, `viewport mobile aplicado: ${desiredViewport.width}x${desiredViewport.height}`);
    }
  ...
  async openCommentsSurface() {
    ...
    // Reels feed controls are often icon-only and have no permalink or article wrapper.
    const visibleReelButton = await this.page.evaluate(() => {
      const viewport = { width: innerWidth, height: innerHeight };
      const videos = [...document.querySelectorAll("video")]
        .map((video) => ({ video, rect: video.getBoundingClientRect() }))
        .filter(({ rect }) => rect.width > 20 && rect.height > 20 && rect.bottom > 0 && rect.top < viewport.height)
        .sort((a, b) => {
          const area = (rect) => rect.width * Math.max(0, Math.min(rect.bottom, viewport.height) - Math.max(rect.top, 0));
          return area(b.rect) - area(a.rect);
        });
      const active = videos[0]?.rect;
      const controls = [...document.querySelectorAll("button, [role='button']")];
      const matches = controls.map((control, index) => {
        const rect = control.getBoundingClientRect();
        const label = [control.getAttribute("aria-label"), control.getAttribute("title"),
          ...[...control.querySelectorAll("svg")].map((svg) => svg.getAttribute("aria-label"))]
          .filter(Boolean).join(" ");
        const visible = rect.width > 0 && rect.height > 0 && rect.right > 0 && rect.left < viewport.width
          && rect.bottom > 0 && rect.top < viewport.height;
        if (!visible || !/comment|coment.rio|comentar/i.test(label)) return null;
        const y = rect.top + rect.height / 2;
        const x = rect.left + rect.width / 2;
        const distance = active
          ? Math.abs(y - (active.top + active.height / 2)) + Math.abs(x - active.right) / 2
          : Math.abs(y - viewport.height / 2);
        return { index, distance };
      }).filter(Boolean).sort((a, b) => a.distance - b.distance);
      return matches[0]?.index ?? -1;
    }).catch(() => -1);
    if (visibleReelButton >= 0) {
      await robustLocatorClick(this.page.locator("button, [role='button']").nth(visibleReelButton));
      await this.page.waitForTimeout(500);
      return true;
    }
  ...
  async findCommentInput() {
    const visibleInputs = [];
    for (const input of await this.page.locator("textarea, [contenteditable='true'], [role='textbox']").all()) {
      ...
      const details = await input.evaluate((element) => {
        ...
        return {
          ...
          label: [element.getAttribute("aria-label"), element.getAttribute("placeholder"), element.getAttribute("data-placeholder")].filter(Boolean).join(" "),
          inDialog: Boolean(element.closest("[role='dialog']"))
        };
      }).catch(() => null);
      if (!details?.inViewport || /search|pesquisar|buscar/i.test(details.label)) continue;
      visibleInputs.push({ input, score: Number(details.inDialog) * 2 + Number(/comment|coment.rio|comentar/i.test(details.label)) });
    }
    visibleInputs.sort((a, b) => b.score - a.score);
    if (visibleInputs.length) return visibleInputs[0].input;
  ...
  async submitComment(commentInput = null) {
    const inputCenter = commentInput?.evaluate ? await commentInput.evaluate((element) => {
      const rect = element.getBoundingClientRect();
      return { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    }).catch(() => null) : null;
    const candidates = [
      this.page.getByRole("button", { name: /^Post$|^Publicar$|^Enviar$/i }),
      this.page.getByText(/^Post$|^Publicar$|^Enviar$/i)
    ];

    for (const group of candidates) {
      const buttons = await group.all();
      const enabled = [];
      for (const button of buttons) {
        if (!await button.isVisible().catch(() => false) || !await button.isEnabled().catch(() => false)) continue;
        const position = await button.evaluate((element) => {
          const rect = element.getBoundingClientRect();
          return { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
        }).catch(() => null);
        const distance = inputCenter && position
          ? Math.abs(position.x - inputCenter.x) + Math.abs(position.y - inputCenter.y)
          : 0;
        enabled.push({ button, distance });
      }
      enabled.sort((a, b) => a.distance - b.distance);
      if (enabled.length) {
        await robustLocatorClick(enabled[0].button);
        return true;
      }
    }
  ...
  async isGoogleChallengePage(page = this.page) {
    if (!page) return false;
    const url = `${page.url?.() || ""}`;
    if (/google\.[^/]+\/(?:sorry|challenge)|recaptcha/i.test(url)) return true;

    const bodyLocator = typeof page.locator === "function" ? page.locator("body") : null;
    const challengeLocator = typeof page.locator === "function"
      ? page.locator("iframe[src*='recaptcha'], iframe[title*='reCAPTCHA'], form[action*='/sorry/'], #captcha-form, [id*='captcha']")
      : null;
    const [title, body, challengeMarkerCount, frameUrls] = await Promise.all([
      typeof page.title === "function" ? Promise.resolve(page.title()).catch(() => "") : "",
      typeof bodyLocator?.innerText === "function" ? bodyLocator.innerText({ timeout: 1800 }).catch(() => "") : "",
      typeof challengeLocator?.count === "function" ? challengeLocator.count().catch(() => 0) : 0,
      Promise.resolve(page.frames?.().map((frame) => frame.url?.() || "") || [])
    ]);
    const visibleText = stripAccents(`${title} ${body}`).toLowerCase();
    return challengeMarkerCount > 0 || frameUrls.some((frameUrl) => /recaptcha|\/sorry\//i.test(frameUrl)) || /(?:nao sou um robo|confirme que voce nao e um robo|verifique se voce e humano|verify you are human|unusual traffic|trafego incomum|consultas automatizadas|automated queries|captcha)/i.test(visibleText);
  }
