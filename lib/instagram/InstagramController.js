  ...
  async getFollowButtonState() {
    ...
    for (const candidate of candidates) {
      const locator = this.page.getByRole("button", {
        name: new RegExp(`^${escapeRegExp(candidate.label)}$`, "i")
      }).first();
    ...
    const profileHeader = this.page.locator("header").first();
    const buttons = await profileHeader.getByRole("button").allTextContents().catch(() => []);
    const fallbackText = buttons.map((text) => text.trim()).filter(Boolean).join(" | ");
