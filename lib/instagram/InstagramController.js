...
export function selectGuidedGoogleResult(rawResults = [], { preferNews = false } = {}) {
  ...
  if (!preferNews) return candidates[0];
  return candidates.find(({ url, text }) => (
    /\b(not[ií]cia|jornal|reportagem|pol[ií]tica|elei[cç][aã]o|candidato)\b/iu.test(`${text} ${url}`)
  )) || candidates[0];
}
...
export class InstagramController {
  ...
  async searchPublicWeb(query, { preferGoogle = true, news = false } = {}) {
    ...
    const fallbackQuery = news ? `${query} notícia` : query;
    await this.page.goto(`https://www.bing.com/search?q=${encodeURIComponent(fallbackQuery)}&setlang=pt-br`, {
      waitUntil: "domcontentloaded",
      timeout: 20000
    });
    await this.page.waitForTimeout(1200);
  ...
  startGoogleGuidance(guidance = "") {
    ...
    void this.followGoogleGuidance(plan, generation);
  ...
  async followGoogleGuidance(planInput, generation = ++this.googleGuidanceGeneration) {
    ...
    try {
      ...
      if (plan.openResult && generation === this.googleGuidanceGeneration) {
        const selected = selectGuidedGoogleResult(await this.collectCurrentSearchResults(), plan);
        if (selected) {
          this.lastAction = "google_guidance_open_result";
          this.research = { ...this.research, step: "opening_result", selectedUrl: selected.url };
          this.updateStatus(INSTAGRAM_STATUS.NAVIGATING, "GOOGLE: ABRINDO RESULTADO ESCOLHIDO");
          await this.page.goto(selected.url, { waitUntil: "domcontentloaded", timeout: 20000 });
          await this.page.waitForTimeout(900);
        }
      }

      if (plan.durationSeconds > 0 && generation === this.googleGuidanceGeneration && this.research.selectedUrl) {
        this.lastAction = "google_guidance_read";
        this.research.step = "reading";
        this.updateStatus(INSTAGRAM_STATUS.ACTING, `GOOGLE: LENDO POR ${plan.durationSeconds} SEGUNDOS`);
        const startedAt = Date.now();
        while (generation === this.googleGuidanceGeneration && Date.now() - startedAt < plan.durationSeconds * 1000) {
          await this.page.waitForTimeout(Math.min(3000, Math.max(1, (plan.durationSeconds * 1000) - (Date.now() - startedAt))));
          await this.page.evaluate(() => window.scrollBy({ top: Math.round(window.innerHeight * 0.55), behavior: "smooth" })).catch(() => {});
        }
      }

      if (generation !== this.googleGuidanceGeneration) return { status: "stopped", message: "GOOGLE: orientação interrompida" };
      this.currentUrl = this.page.url();
      this.research.step = this.research.selectedUrl ? "result_ready" : "results_ready";
      this.updateStatus(INSTAGRAM_STATUS.READY, this.research.selectedUrl ? "GOOGLE: RESULTADO ABERTO" : "GOOGLE: RESULTADOS ABERTOS");
      return { status: "ready", message: "GOOGLE: orientação concluída", plan, url: this.currentUrl };
    } catch (error) {
      const message = publicError(error);
      this.lastError = message;
      if (this.research) this.research.step = "error";
