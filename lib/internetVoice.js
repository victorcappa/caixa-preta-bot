...
function inferHumorMechanisms(text = "") {
  ...
  if (/\b(?:ninguem|metade|mao|palma|silencio|segundos|olhou|demorou|subiu|correu|esperou)\b/.test(normalized)) mechanisms.push("specific_observation");
  ...
  if (/\b(?:eu vi|voces escolheram|nem terminei|mudou de resposta|ninguem perguntou|muito rapido)\b/.test(normalized)) mechanisms.push("social_exposure");
  if (/\b(?:ta|ok|bom|foi isso|pior que|infelizmente)\b/.test(normalized)) mechanisms.push("understatement");
...
function buildSystemContextFromGuide(guide) {
  return [
    ...
    "- REACAO > PIADA ESCRITA. Soe como quem estava ali, viu e comentou antes de pensar demais.",
    ...
    "- Humor nasce de detalhe observavel, exposicao social ou subestimacao. Nao invente metafora, diagnostico ou aleatoriedade para fabricar personalidade.",
    "- Fala funcional pode ser totalmente seca: vai; acabou; abre; esquerda ou direita; olha pra tela.",
    "- Separe INSTRUCAO, ESPERA e REACAO em turnos. Evite empilhar instrucao + piada + explicacao na mesma fala.",
    "- Se uma versao 50% menor funcionar, use a menor.",
    "- Nao use X e/ou Y como punchline. Nao termine observacoes com duas explicacoes alternativas engracadinhas.",
    "- Evite como cacoete: caos, energia caotica, vibes, material, atencao disponivel, vontade detectada, silencio dramatico detectado, interessante escolha e curiosa decisao.",
    "- Nao use giria marcada por iniciativa propria para demonstrar repertorio. So use se o acontecimento concreto realmente pedir aquela expressao.",
...
function chooseProfile({ activeText, conversation, memories }) {
  ...
  if (signals.quickReaction || signals.quiet || signals.singleResponse) {
    ...
    humor.add("specific_observation");
  }
  ...
  if (signals.dataCollection) {
    humor.add("social_exposure");
    humor.add("understatement");
  }
  ...
  if (!humor.size) humor.add("understatement");
...
export function buildInternetVoiceContext({
  ...
  const productiveVerbs = [];
  // Giria marcada nao entra como menu automatico. O modelo nao deve procurar
  // "qual termo jovem cabe aqui"; o registro funciona por ritmo e reacao.
