import { SCENE_ZERO_EVIDENCIAS_CHALLENGE, SCENE_ZERO_GINCANAS, SCENE_ZERO_SMELL_CHALLENGE } from "../../data/scene-zero-gincanas.js";
...
export const SCENE_ZERO_SUITCASES = {
  1: { id: 1, label: "NOVA BIOS / MOREL", game: "morel_bios" },
...
export function buildSuitcaseSelectionCue(suitcaseNumber) {
  ...
  return `Vá até a mala indicada: ${number}. A luz vai indicar.`;
}
...
export function sceneZeroGincanaDuration(task, value) {
  const fixedTwentySeconds = [SCENE_ZERO_FIRST_CHALLENGE.id, SCENE_ZERO_SECOND_CHALLENGE.id].includes(task?.id);
  return clampGincanaDuration(value, fixedTwentySeconds ? 20 : 60);
}
