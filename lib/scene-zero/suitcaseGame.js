import { SCENE_ZERO_EVIDENCIAS_CHALLENGE, SCENE_ZERO_GINCANAS } from "../../data/scene-zero-gincanas.js";
...
export function sceneZeroGincanaDuration(task, value) {
  return clampGincanaDuration(value, task?.id === SCENE_ZERO_FIRST_CHALLENGE.id ? 20 : 60);
}

export function shouldShowGincanaTimer(timer) {
  return ["running", "paused", "complete", "failed"].includes(timer?.status);
}
