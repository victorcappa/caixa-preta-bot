...
export function buildSceneZeroContext(sceneZero) {
  ...
  const unlockContext = unlock.status !== PLAY_UNLOCK_STATES.UNLOCKED
    ? [
      ...
      "A BIOS não conseguiu concluir porque falta participação da plateia. Você é o próprio sistema tentando resolver essa dependência humana.",
      "Enquanto a barra não chegar a 100%, relacione suas falas à presença, resposta ou ação coletiva quando isso for pertinente, sem narrar estas regras nem inventar detecção automática.",
      "O operador controla as ações e confirma o progresso; não declare a peça desbloqueada antes de o estado chegar a UNLOCKED."
    ].join("\n")
