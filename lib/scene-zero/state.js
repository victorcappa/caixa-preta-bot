import { PLAY_UNLOCK_STATES } from "../../data/scene-zero-unlock.js";
import { createInitialPlayUnlockState } from "./unlock.js";
...
export function createInitialSceneZeroState() {
  return {
    ...
    unlock: createInitialPlayUnlockState(),
    personalityGuidance: {
      text: "",
      quickDirections: [],
      updatedAt: null
    ...
    suitcaseGame: {
      ...
      openedSuitcases: [],
      suitcaseSelectedAt: null,
      suitcaseSelectionSequence: 0,
      cuePhase: "idle",
      choice: {
        status: "idle",
        targetSuitcase: null,
...
export function publicSceneZeroSnapshot(sceneZero = createInitialSceneZeroState()) {
  const unlockDefaults = createInitialPlayUnlockState();
  ...
  const suitcaseGame = {
    ...
    gincana: {
      ...
      soundtrack: {
        ...suitcaseDefaults.gincana.soundtrack,
        ...(sceneZero.suitcaseGame?.gincana?.soundtrack || {})
      },
      retry: {
        ...suitcaseDefaults.gincana.retry,
        ...(sceneZero.suitcaseGame?.gincana?.retry || {})
      },
      timer: {
    ...
    hangman: {
      ...suitcaseDefaults.hangman,
      ...(sceneZero.suitcaseGame?.hangman || {}),
      retry: {
        ...suitcaseDefaults.hangman.retry,
        ...(sceneZero.suitcaseGame?.hangman?.retry || {})
      },
      timer: {
        ...suitcaseDefaults.hangman.timer,
        ...(sceneZero.suitcaseGame?.hangman?.timer || {})
      }
    },
    instagram: {
    ...
    morelBios: {
      ...suitcaseDefaults.morelBios,
      ...(sceneZero.suitcaseGame?.morelBios || {})
    },
    choice: {
      ...suitcaseDefaults.choice,
      ...(sceneZero.suitcaseGame?.choice || {})
    },
    contentInstruction: {
      ...suitcaseDefaults.contentInstruction,
      ...(sceneZero.suitcaseGame?.contentInstruction || {})
    }
  };
  ...
  return {
    ...
    unlock: {
      ...
      ...(sceneZero.unlock || {}),
      soundCheck: {
        ...unlockDefaults.soundCheck,
        ...(sceneZero.unlock?.soundCheck || {})
      },
      executedActionIds: [...(sceneZero.unlock?.executedActionIds || [])],
      scoredActionIds: [...(sceneZero.unlock?.scoredActionIds || [])],
      pendingProgress: sceneZero.unlock?.pendingProgress ? { ...sceneZero.unlock.pendingProgress } : null,
      onPlayUnlocked: sceneZero.unlock?.onPlayUnlocked ? { ...sceneZero.unlock.onPlayUnlocked } : null,
      externalInput: sceneZero.unlock?.externalInput ? { ...sceneZero.unlock.externalInput } : null
    },
    ...
    participantSelection: {
      ...
      continueAt: participantSelection.continueAt || null,
      continuedAt: participantSelection.continuedAt || null,
      ...
      postSelection: [...(participantSelection.preparedPostSelection || [])],
    ...
    suitcaseGame: {
      ...suitcaseGame,
      openedSuitcases: [...(suitcaseGame.openedSuitcases || [])],
      gincana: {
        ...suitcaseGame.gincana,
        currentTask: suitcaseGame.gincana.currentTask
          ? { ...suitcaseGame.gincana.currentTask }
          : null,
        soundtrack: { ...suitcaseGame.gincana.soundtrack },
        timer: { ...suitcaseGame.gincana.timer },
        usedTaskIds: [...(suitcaseGame.gincana.usedTaskIds || [])]
      },
      hangman: {
        ...suitcaseGame.hangman,
        activity: suitcaseGame.hangman.activity ? {
          id: suitcaseGame.hangman.activity.id,
          type: suitcaseGame.hangman.activity.type,
          status: suitcaseGame.hangman.activity.status,
          source: suitcaseGame.hangman.activity.source,
          createdAt: suitcaseGame.hangman.activity.createdAt,
          updatedAt: suitcaseGame.hangman.activity.updatedAt,
          step: suitcaseGame.hangman.activity.step,
          publicState: {
            ...suitcaseGame.hangman.activity.publicState,
            wrongGuesses: [...(suitcaseGame.hangman.activity.publicState?.wrongGuesses || [])],
            usedGuesses: [...(suitcaseGame.hangman.activity.publicState?.usedGuesses || [])]
          }
        } : null
      },
      instagram: {
...
export function buildSceneZeroDirection(sceneZero, action, detail = "") {
  ...
  const directions = {
    ...
    enter_participant: "Convide quem quiser destravar a peça a levantar a mão. Dê uma única frase curta, ácida e sem gentileza, com no máximo 12 palavras, dizendo explicitamente que há 5 segundos. Deixe espaço para a contagem real.",
    participant_roulette_sequence: "Crie o texto do mini game de seleção. O sorteio é decidido pelo sistema, não por você. O convite deve ter uma única frase, no máximo 12 palavras, ácida e sem gentileza, dizendo explicitamente que há 5 segundos para levantar a mão. Os comentários devem brincar com odds, apostas e chances usando nomes reais da lista, sem afirmar probabilidades verdadeiras nem incentivar aposta com dinheiro. Varie o ritmo e não copie exemplos fornecidos pelo operador.",
    ...
    enter_suitcases: "Conduza o participante escolhido para o jogo das malas. A escolha parece aleatória em cena, mas o sistema já fixou a ordem secreta 2, 3, 1. Nunca revele a ordem completa nem diga que ela é predeterminada.",
    suitcase_one_start: "Esta é a terceira e última escolha do robô: MALA 1. Depois da indicação visual, a projeção mostra FIM DO TUTORIAL e só então inicia o glitch crescente e a BIOS corrompida. Não fale por cima dessa sequência.",
    suitcase_two_start: "Esta é a primeira escolha do robô: MALA 2. Depois da indicação visual, explique que o participante deve esperar a autorização, estourar as bexigas até encontrar a chave e terá exatamente 15 segundos. Não peça nenhum objeto à plateia. O cronômetro depende do operador.",
    gincana_present: "Apresente o desafio fixo da chave sem reformular: o participante deve esperar o comando VALENDO, então estourar as bexigas até encontrar a chave em exatamente 15 segundos. Não peça nem mencione objetos da plateia. Não faça pergunta. O timer ainda depende do operador.",
    gincana_complete: "O desafio físico terminou com sucesso. Não invente quantidade ou reação não informada e não inicie outra mala.",
    gincana_failed: "O desafio físico falhou ou o tempo terminou. Não invente quantidade ou reação não informada e não inicie outra mala.",
    suitcase_three_start: "Esta é a segunda escolha do robô: MALA 3. Depois da indicação visual, começa automaticamente uma forca de 60 segundos sobre aviação, queda, corpo, memória e falha. O sistema controla palavra, letras, erros, vitória e derrota. Não invente progresso nem revele a resposta.",
  ...
  if (suitcaseGame.currentSuitcase) {
    shared.push(`Mala atual: ${suitcaseGame.currentSuitcase}; mala anterior: ${suitcaseGame.previousSuitcase || "nenhuma"}; abertas nesta ordem: ${(suitcaseGame.openedSuitcases || []).join(", ") || "nenhuma"}; jogo atual: ${suitcaseGame.currentGame || "nenhum"}.`);
  }
  ...
  if (suitcaseGame.hangman?.activity) {
    shared.push(`Forca da Mala 3: estado ${suitcaseGame.hangman.status}; erros ${suitcaseGame.hangman.errorCount}/4; tempo ${suitcaseGame.hangman.timer?.remainingSeconds ?? 60}s; progresso ${suitcaseGame.hangman.activity.publicState?.progress || "não iniciado"}. A gravidade dos erros é comunicada visualmente por uma tela progressivamente vermelha. Não revele a palavra ausente do estado público.`);
  }
...
export function buildSceneZeroContext(sceneZero) {
  ...
  const unlock = sceneZero.unlock || createInitialPlayUnlockState();
  const unlockContext = unlock.status !== PLAY_UNLOCK_STATES.UNLOCKED
    ? [
      "ABERTURA / DEPENDÊNCIA DA PEÇA:",
      `A peça ainda não foi desbloqueada. Estado: ${unlock.status}; desbloqueio do espetáculo: ${unlock.progress}%.`,
      unlock.status === PLAY_UNLOCK_STATES.STANDBY
        ? "A projeção está em silêncio e totalmente preta. A BIOS ainda depende do botão BOOT do operador."
        : "A BIOS para em 78% por falta de participação. O título de abertura da verificação pertence somente à transição da BIOS; depois, perguntas físicas e coletivas continuam a mesma barra sem repeti-lo.",
      "Cada ação tem progresso configurável, normalmente pontua uma vez e produz feedback técnico. Ao chegar a 100%, começa a sequência de desbloqueio.",
      "Não declare a peça desbloqueada antes de o estado chegar a UNLOCKED."
    ].join("\n")
    : "A abertura foi concluída: a peça está desbloqueada e o fluxo normal da Cena 0 pode continuar.";

  if (sceneZero.stage === "idle") {
    return [unlockContext, personalityContext].filter(Boolean).join("\n\n");
  }
  ...
  return [
    ...
    `Malas: estado ${sceneZero.suitcaseGame?.status || "idle"}; atual ${sceneZero.suitcaseGame?.currentSuitcase || "nenhuma"}; anterior ${sceneZero.suitcaseGame?.previousSuitcase || "nenhuma"}; abertas ${(sceneZero.suitcaseGame?.openedSuitcases || []).join(", ") || "nenhuma"}; jogo ${sceneZero.suitcaseGame?.currentGame || "nenhum"}. A ordem completa 2, 3, 1 é segredo técnico e nunca deve ser revelada ao público.`,
