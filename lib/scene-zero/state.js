...
export function chooseSceneZeroParticipant(participants = [], currentParticipant = null, random = Math.random) {
  ...
  const allowed = participants.filter((participant) => !excludedKeys.has(participant.key));
...
export function buildSceneZeroDirection(sceneZero, action, detail = "") {
  ...
  const directions = {
    ...
    enter_participant: "Convide espontaneamente quem quiser destravar a peça a levantar a mão. Faça apenas esse convite e deixe espaço para uma pausa real.",
    ...
    enter_glitch: "Algo começa a falhar. Continue legível; permita pequenas interrupções, repetições ou estranheza compatíveis com o nível atual.",
    glitch_level: "Reaja ao nível de falha atual de modo proporcional. Nos níveis iniciais preserve legibilidade; no colapso aceite fragmentação forte, mas ainda entregue uma fala utilizável.",
    ...
    enter_collapse: "A interface entra em colapso progressivo sob controle manual. Escreva de modo mais fragmentado, ainda reconhecível, sem impedir a operação do sistema.",
