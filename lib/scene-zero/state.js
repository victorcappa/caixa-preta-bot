...
export function buildSceneZeroDirection(sceneZero, action, detail = "") {
  ...
  const directions = {
    ...
    enter_suitcases: "Conduza o participante escolhido para o jogo das malas. A escolha parece aleatória em cena, mas o sistema já fixou a ordem secreta 1, 2, 3. Nunca revele a ordem completa nem diga que ela é predeterminada.",
    suitcase_one_start: "Esta é a primeira escolha do robô: MALA 1. Depois da indicação visual, a projeção apresenta um desafio físico curto contra o tempo. O texto, o alvo e a duração são publicados pelo sistema e não podem ser reformulados. O cronômetro depende do operador.",
    suitcase_two_start: "Esta é a segunda escolha do robô: MALA 2. Depois da indicação visual, começa uma forca sobre aviação, queda, corpo, memória e falha. O sistema controla palavra, letras, erros e resultado. Não invente progresso nem revele a resposta.",
    ...
    suitcase_three_start: "Esta é a terceira e última escolha do robô: MALA 3. Depois da indicação visual, o robô sofre o glitch crescente já configurado e uma nova BIOS corrompida ocupa a projeção até tudo ficar preto. Não nomeie a referência literária e não fale por cima da BIOS.",
  ...
  if (suitcaseGame.hangman?.activity) {
    shared.push(`Forca da Mala 2: estado ${suitcaseGame.hangman.status}; voo ${suitcaseGame.hangman.flightState}; erros ${suitcaseGame.hangman.errorCount}/4; progresso ${suitcaseGame.hangman.activity.publicState?.progress || "não iniciado"}. Não revele a palavra ausente do estado público.`);
  }
