...
export function buildSceneZeroDirection(sceneZero, action, detail = "") {
  ...
  const directions = {
    ...
    suitcase_two_start: "Esta é a primeira escolha do robô: MALA 2. Depois da indicação visual, a projeção apresenta um desafio com objetos obtidos com ajuda da plateia e encerra sempre com todos fingindo estar mortos nas cadeiras e no chão por 30 segundos. O texto, o alvo e a duração são publicados pelo sistema e não podem ser reformulados. O cronômetro depende do operador.",
    gincana_present: "Apresente ao participante a gincana sorteada como uma ordem fechada, assertiva e imediatamente executável, sem mudar objetivo, itens, quantidades, critérios, limites de segurança ou duração. Diga claramente quanto tempo existe. Determine exatamente o que deve ser trazido ou feito: não ofereça alternativas, categorias, exemplos, opções, liberdade de escolha nem expressões como 'você escolhe', 'por exemplo', 'pode ser' ou 'o que preferir'. Não faça pergunta. O timer ainda depende do operador.",
  ...
  if (suitcaseGame.hangman?.activity) {
    shared.push(`Forca da Mala 3: estado ${suitcaseGame.hangman.status}; voo ${suitcaseGame.hangman.flightState}; erros ${suitcaseGame.hangman.errorCount}/4; tempo ${suitcaseGame.hangman.timer?.remainingSeconds ?? 60}s; progresso ${suitcaseGame.hangman.activity.publicState?.progress || "não iniciado"}. Não revele a palavra ausente do estado público.`);
  }
