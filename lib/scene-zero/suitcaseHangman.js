...
export const SCENE_ZERO_HANGMAN_RETRY_DURATION_SECONDS = 30;
...
export function createInitialSceneZeroHangmanState() {
  return {
    ...
    retry: {
      status: "idle",
      messageId: null,
      durationSeconds: SCENE_ZERO_HANGMAN_RETRY_DURATION_SECONDS,
      count: 0
    },
...
export function prepareSceneZeroHangmanRetry(current, durationSeconds = SCENE_ZERO_HANGMAN_RETRY_DURATION_SECONDS) {
  if (!current?.activity) return null;
  const now = new Date().toISOString();
  const safeDuration = Math.max(1, Math.round(Number(durationSeconds) || SCENE_ZERO_HANGMAN_RETRY_DURATION_SECONDS));
...
export function restartSceneZeroHangmanRetry(current) {
  if (current?.status !== "retry_wait" || !["announcing", "ready"].includes(current.retry?.status) || !current.activity) return null;
  const now = new Date();
  const durationSeconds = Math.max(1, Math.round(Number(current.retry.durationSeconds) || SCENE_ZERO_HANGMAN_RETRY_DURATION_SECONDS));
  return {
    ...current,
    status: "active",
